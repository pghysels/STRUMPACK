...
test(int argc, char* argv[], CSRMatrix<scalar,integer>& A) {
  ...
  if (!rank) std::cout << "# COMPONENTWISE SCALED RESIDUAL = "
...
int main(int argc, char* argv[]) {
  ...
  if (is_complex) {
    ...
    MPI_Bcast(A_c->get_ptr(), n+1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(A_c->get_ind(), nnz, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(A_c->get_val(), nnz, MPI_C_DOUBLE_COMPLEX, 0, MPI_COMM_WORLD);
    test(argc, argv, *A_c);
    delete A_c;
  } else {
    if (rank) A = new CSRMatrix<double,int>(n, nnz);
    MPI_Bcast(A->get_ptr(), n+1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(A->get_ind(), nnz, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(A->get_val(), nnz, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    test(argc, argv, *A);
    delete A;
  }
  TimerList::Finalize();
