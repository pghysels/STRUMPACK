...
def quality(p, l):
    miss = sum(p[i]*l[i] < 0 for i in range(m))
    return 100.*(m - miss) / m

# Kernel ridge regression classification
K = sp.STRUMPACKKernel(
    h, lam, degree, kernel='rbf', approximation='HSS', argv=sys.argv)
K.fit(train_points, train_labels)
K.classes_
pred = K.predict(test_points)
# check quality, labels are -1 or +1
print('HSS KernelRR quality =', quality(pred, test_labels), '%')

print("classes:", K.classes_)
