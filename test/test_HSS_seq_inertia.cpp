...
int run(int argc, char* argv[]) {
  ...
  auto H2 = H.dense();
  auto HT = H2.transpose();
  H2.scaled_add(-1., HT);
  auto AT = A.transpose();
  AT.scaled_add(-1., A);
  cout << "# symmetry error = ||H - H^T||_2 = " << H2.norm()  << endl;
  cout << "# symmetry error = ||H - H^T||_1 = " << H2.norm1() << endl;
  cout << "# symmetry error = ||H - H^T||_I = " << H2.normI() << endl;
  cout << "# symmetry error = ||H - H^T||_F = " << H2.normF() << endl;
  cout << "#                  ||A - A^T||_F = " << AT.normF() << endl;


  // Inertia Tests
  cout << "# computing inertia of HSS matrix .." << endl;
  auto in = H.inertia();
  cout << "# Positive Eigval: " << in.np << endl;
  cout << "# Negative Eigval: " << in.nn << endl;
  cout << "#     Zero Eigval: " << in.nz << endl;
  // End Inertia Tests
