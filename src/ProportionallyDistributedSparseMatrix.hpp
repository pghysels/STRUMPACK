...
namespace strumpack {
  ...
  class ProportionallyDistributedSparseMatrix : public CompressedSparseMatrix<scalar_t,integer_t> {
  ...
  public:
    ...
    ProportionallyDistributedSparseMatrix<scalar_t,integer_t>* clone() { return NULL; }
    ...
    real_t max_scaled_residual(scalar_t* x, scalar_t* b) { return real_t(1.); };

  protected:
    integer_t _local_cols;  // number of columns stored on this proces
    integer_t* _global_col; // for each local column, this gives the global column index
  };

  template<typename scalar_t,typename integer_t>
  ProportionallyDistributedSparseMatrix<scalar_t,integer_t>::ProportionallyDistributedSparseMatrix()
    : CompressedSparseMatrix<scalar_t,integer_t>(), _global_col(NULL) { }
  ...
  (CSRMatrixMPI<scalar_t,integer_t>* Ampi, MatrixReorderingMPI<scalar_t,integer_t>* nd,
   EliminationTreeMPIDist<scalar_t,integer_t>* et, bool duplicate_fronts) {
    this->_n = Ampi->size();
    this->_nnz = Ampi->nnz();
    auto comm = Ampi->comm();
...
#pragma omp atomic
    ...
    std::vector<Triplet> triplets(std::accumulate(rcnts, rcnts+P, 0)/sizeof(Triplet));
    MPI_Alltoallv(sbuf, scnts, sdisp, MPI_BYTE, triplets.data(), rcnts, rdisp, MPI_BYTE, comm);
	...
	if (a.c != b.c) return (a.c < b.c);  // sort according to column, then rows
	return (a.r < b.r);
      });
#else
  ...
  (integer_t sep_end, const std::vector<std::size_t>& I, const std::vector<std::size_t>& J,
   DenseM_t& B, int depth) const {
	...
	if (r >= rmin && r <= rmax && (_global_col[c] < sep_end || r < sep_end)) {
	  ...
	  B(i,j) = (this->_ind[a_pos] == r) ? this->_val[a_pos] : scalar_t(0.);
  ...
  (integer_t slo, integer_t shi, integer_t* upd, integer_t dupd,
   const DenseM_t& R, DenseM_t& Sr, DenseM_t& Sc) const {
    auto ds = shi - slo;
    auto nbvec = R.cols();
    auto c = std::lower_bound(_global_col, _global_col+_local_cols, slo) - _global_col;
    auto chi = std::lower_bound(_global_col+c, _global_col+_local_cols, shi) - _global_col;
    for (; c<chi; c++) {
    ...
    for (integer_t i=0; i<dupd; i++) { // update columns
      ...
      c = std::lower_bound(_global_col+c, _global_col+_local_cols, upd[i]) - _global_col;
  ...
  (scalar_t* F11, scalar_t* F12, scalar_t* F21, integer_t dim_sep, integer_t dim_upd,
   integer_t sep_begin, integer_t sep_end, integer_t* upd, int depth) {
    auto c = std::lower_bound(_global_col, _global_col+_local_cols, sep_begin) - _global_col;
    auto chi = std::lower_bound(_global_col+c, _global_col+_local_cols, sep_end) - _global_col;
    for (; c<chi; c++) {
      auto col = _global_col[c];
      integer_t row_ptr = 0;
      for (integer_t j=this->_ptr[c]; j<this->_ptr[c+1]; j++) {
	...
	if (row >= sep_begin) {
	  if (row < sep_end) F11[row-sep_begin + (col-sep_begin)*dim_sep] = this->_val[j];
	    ...
	    if (upd[row_ptr] == row) F21[row_ptr + (col-sep_begin)*dim_upd] = this->_val[j];
	  }
	}
      }
    }
  ...
  (scalar_t* F, integer_t ldF, integer_t row, integer_t nr_rows,
   integer_t col, integer_t nr_cols) {
    auto c = std::lower_bound(_global_col, _global_col+_local_cols, col) - _global_col;
    auto chi = std::lower_bound(_global_col+c, _global_col+_local_cols, col+nr_cols) - _global_col;
    for (; c<chi; c++) {
      for (integer_t j=this->_ptr[c]; j<this->_ptr[c+1]; j++) {
    	auto r = this->_ind[j];
    	if (r >= row) {
    	  if (r < row + nr_rows) F[r-row + (_global_col[c]-col)*ldF] = this->_val[j];
    	  else break;
    	}
      }
    }
  ...
  (scalar_t* F, integer_t ldF, integer_t row, integer_t nr_rows,
   integer_t col, integer_t nr_cols, integer_t* upd) {
    ...
    auto c = std::lower_bound(_global_col, _global_col+_local_cols, upd[0]) - _global_col;
    for (integer_t i=0; i<nr_cols; i++) {
      //while (c < _local_cols && _global_col[c] < upd[i]) c++;
      if (i > 0) c = std::lower_bound(_global_col+c, _global_col+_local_cols, upd[i]) - _global_col;
      if (c == _local_cols || _global_col[c] != upd[i]) continue;
      for (integer_t j=this->_ptr[c]; j<this->_ptr[c+1]; j++) {
	auto r = this->_ind[j];
	if (r >= row) {
   ...
   integer_t col, integer_t nr_cols, integer_t* upd) {
    ...
    for (; c<chi; c++) {
      ...
      for (integer_t j=this->_ptr[c]; j<this->_ptr[c+1]; j++) {
    	...
    	if (upd[row_upd] == r) F[row_upd + (_global_col[c]-col)*ldF] = this->_val[j];
      }
    }
  }
  ...
  template<typename scalar_t,typename integer_t> void
  ProportionallyDistributedSparseMatrix<scalar_t,integer_t>::extract_separator_2d
  (integer_t sep_end, const std::vector<std::size_t>& I, const std::vector<std::size_t>& J,
   DistM_t& B, MPI_Comm comm) const {
    if (!B.active()) return;
    auto m = B.rows();
  ...
  (integer_t sep_begin, integer_t sep_end, integer_t* upd, integer_t dim_upd,
   DistM_t& R, DistM_t& Srow, DistM_t& Scol, int ctxt_all, MPI_Comm R_comm, int depth) {
    ...
    auto clo = std::lower_bound(_global_col, _global_col+_local_cols, sep_begin) - _global_col;
    auto chi = std::lower_bound(_global_col+clo, _global_col+_local_cols, sep_end) - _global_col;
    integer_t c = clo;
    for (; c<chi; c++) { // separator columns
      auto row_j_rank = R.rowg2p(_global_col[c] - sep_begin);
      integer_t row_upd = 0;
      for (integer_t j=this->_ptr[c]; j<this->_ptr[c+1]; j++) {
	auto row = this->_ind[j];
	if (row >= sep_begin) {
	  if (row < sep_end) {
	    auto row_i_rank = R.rowg2p(row - sep_begin);
	    if (row_j_rank == p_row) { rows_to[row_i_rank]++; rows_from[row_i_rank]++; }
	    if (row_i_rank == p_row) { rows_to[row_j_rank]++; rows_from[row_j_rank]++; }
	    local_flops += 4 * lcols;
	  } else {
	    while (row_upd < dim_upd && upd[row_upd] < row) row_upd++;
	    if (row_upd == dim_upd) break;
	    if (upd[row_upd] == row) {
	      auto row_i_rank = R.rowg2p(dim_sep + row_upd);
	      if (row_j_rank == p_row) { rows_to[row_i_rank]++; rows_from[row_i_rank]++; }
	      if (row_i_rank == p_row) { rows_to[row_j_rank]++; rows_from[row_j_rank]++; }
	      local_flops += 4 * lcols;
	    }
    ...
    for (integer_t i=0; i<dim_upd; i++) { // update columns
      c = std::lower_bound(_global_col+c, _global_col+_local_cols, upd[i]) - _global_col;
      if (c == _local_cols || _global_col[c] != upd[i]) continue;
      auto row_j_rank = R.rowg2p(dim_sep + i);
      for (integer_t j=this->_ptr[c]; j<this->_ptr[c+1]; j++) {
    ...
    pp[0] = sbuf;
    for (integer_t p=1; p<p_rows; p++) pp[p] = pp[p-1] + rows_to[p-1]*lcols;

    for (c=clo; c<chi; c++) { // separator columns
      auto Aj = _global_col[c] - sep_begin;
      auto row_j_rank = R.rowg2p(Aj);
      integer_t row_upd = 0;
      for (integer_t j=this->_ptr[c]; j<this->_ptr[c+1]; j++) {
	auto row = this->_ind[j];
	if (row >= sep_begin) {
	  if (row < sep_end) {
	    auto a = this->_val[j];
	    auto Ai = row - sep_begin; auto row_i_rank = R.rowg2p(Ai);
	    if (row_j_rank == p_row) {
	      for (integer_t k=0, r=R.rowg2l(Aj); k<lcols; k++) pp[row_i_rank][k] += a * R(r, k);
	      pp[row_i_rank] += lcols;
	    }
	    if (row_i_rank == p_row) { // transpose
	      for (integer_t k=0, r=R.rowg2l(Ai); k<lcols; k++) pp[row_j_rank][k] += a * R(r, k);
	      pp[row_j_rank] += lcols;
	    }
	  } else {
	    while (row_upd < dim_upd && upd[row_upd] < row) row_upd++;
    ...
    // TODO instead of a send/receive loop, get the column communicator from BLACS?
    ...
    for (integer_t p=0; p<p_rows; p++)
      MPI_Isend(pp[p], rows_to[p]*lcols, mpi_type<scalar_t>(), p+p_col*p_rows, 0, R_comm, sreq+p);

    pp[0] = rbuf;
    for (integer_t p=1; p<p_rows; p++) pp[p] = pp[p-1] + rows_from[p-1]*lcols;
    for (integer_t p=0; p<p_rows; p++)
      MPI_Irecv(pp[p], rows_from[p]*lcols, mpi_type<scalar_t>(), p+p_col*p_rows, 0, R_comm, rreq+p);
	...
	if (row >= sep_begin) {
	  if (row < sep_end) {
	    ...
	    auto row_i_rank = R.rowg2p(Ai);
	    if (row_i_rank == p_row) {
	      for (integer_t k=0, r=Srow.rowg2l(Ai); k<lcols; k++) Srow(r, k) += pp[row_j_rank][k];
	      pp[row_j_rank] += lcols;
	    }
	    if (row_j_rank == p_row) { // transpose
	      for (integer_t k=0, r=Srow.rowg2l(Aj); k<lcols; k++) Scol(r, k) += pp[row_i_rank][k];
	      pp[row_i_rank] += lcols;
	    }
	  } else {
	    while (row_upd < dim_upd && upd[row_upd] < row) row_upd++;
		...
		pp[row_j_rank] += lcols;
	      ...
	      if (row_j_rank == p_row) { // transpose
		for (integer_t k=0, r=Scol.rowg2l(Aj); k<lcols; k++) Scol(r, k) += pp[row_i_rank][k];
		pp[row_i_rank] += lcols;
	      }
	    }
	  }
