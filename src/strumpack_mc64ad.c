...
/* strumpack_mc64ad.f -- translated by f2c (version 20100827).
...
/* CCCC PACKAGE STRUMPACK_MC64A/AD */
...
/* Subroutine */ int_t strumpack_mc64id_(int_t *icntl)
{
    int_t i__;
...
/* *** reliability of the subroutines in this STRUMPACK_MC64 suite, we         *** */
...
/*  The components of the array ICNTL control the action of STRUMPACK_MC64A/AD. */
...
/*    ICNTL(5) to ICNTL(10) are not used by STRUMPACK_MC64A/AD but are set to */
...
/* ********************************************************************** */
/* Subroutine */ int_t strumpack_mc64ad_(int_t *job, int_t *n, int_t *ne, int_t *
	...
	icntl, int_t *info)
{
    ...
    extern /* Subroutine */ int_t mc21ad_(int_t *, int_t *, int_t *, 
	    int_t *, int_t *, int_t *, int_t *, int_t *), strumpack_mc64bd_(
	    int_t *, int_t *, int_t *, int_t *, double *, int_t 
	    *, int_t *, int_t *, int_t *, int_t *, int_t *, 
	    double *), strumpack_mc64rd_(int_t *, int_t *, int_t *, int_t *,
	     double *), strumpack_mc64sd_(int_t *, int_t *, int_t *, int_t *
	    , double *, int_t *, int_t *, int_t *, int_t *, 
	    int_t *, int_t *, int_t *, int_t *, int_t *), strumpack_mc64wd_(
	    int_t *, int_t *, int_t *, int_t *, double *, int_t 
...
/*   output of STRUMPACK_MC64A/AD and must be set by the user before calling */
/*   STRUMPACK_MC64A/AD. They are not altered by the subroutine. */
...
/* IW(1:5N), DW(1:N) are workspaces */
	strumpack_mc64bd_(n, ne, &ip[1], &irn[1], &a[1], &cperm[1], num, &iw[1], &iw[*n 
...
/* Sort entries in each column by decreasing value. */
	strumpack_mc64rd_(n, ne, &ip[1], &iw[1], &dw[1]);
/* IW(NE+1:NE+10N) is workspace */
	strumpack_mc64sd_(n, ne, &ip[1], &iw[1], &dw[1], &cperm[1], num, &iw[*ne + 1], &
...
/* B = DW(2N+1:2N+NE); IW(1:5N) and DW(1:2N) are workspaces */
	strumpack_mc64wd_(n, ne, &ip[1], &irn[1], &dw[(*n << 1) + 1], &cperm[1], num, &
...
/* B = DW(3N+1:3N+NE); IW(1:5N) and DW(1:2N) are workspaces */
	strumpack_mc64wd_(n, ne, &ip[1], &irn[1], &dw[*n * 3 + 1], &cperm[1], num, &iw[
...
/* ********************************************************************** */
/* Subroutine */ int_t strumpack_mc64bd_(int_t *n, int_t *ne, int_t *ip, int_t *
	...
	int_t *pr, int_t *q, int_t *l, double *d__)
{
    ...
    double rinf;
    extern /* Subroutine */ int_t strumpack_mc64dd_(int_t *, int_t *, int_t *, 
	    double *, int_t *, int_t *), strumpack_mc64ed_(int_t *, int_t *,
	     int_t *, double *, int_t *, int_t *), strumpack_mc64fd_(int_t *
...
/* N, NE, IP, IRN are described in STRUMPACK_MC64A/AD. */
...
/*      EXTERNAL FD05AD,STRUMPACK_MC64DD,STRUMPACK_MC64ED,STRUMPACK_MC64FD, DLAMCH */
...
/* Add row I to Q, and push it */
		    ++qlen;
		    l[i__] = qlen;
		    strumpack_mc64dd_(&i__, n, &q[1], &d__[1], &l[1], &c__1);
		}
		jj = iperm[i__];
		pr[jj] = j;
	    }
...
/* If Q2 is empty, extract new rows from Q */
		...
		for (idum = 1; idum <= i__3; ++idum) {
		    strumpack_mc64ed_(&qlen, n, &q[1], &d__[1], &l[1], &c__1);
		    l[i__] = 0;
		    --low;
		    q[low] = i__;
		    if (qlen == 0) {
...
/* Delete row I from Q (if necessary); add row I to Q2 */
			if (di != -1.) {
			    strumpack_mc64fd_(&l[i__], &qlen, n, &q[1], &d__[1], &l[1], 
				    &c__1);
			}
			l[i__] = 0;
			--low;
...
/* ********************************************************************** */
/* Subroutine */ int_t strumpack_mc64dd_(int_t *i__, int_t *n, int_t *q, double 
...
/* Variables N,Q,D,L are described in STRUMPACK_MC64B/BD */
...
/* ********************************************************************** */
/* Subroutine */ int_t strumpack_mc64ed_(int_t *qlen, int_t *n, int_t *q, 
...
/* Variables QLEN,N,Q,D,L are described in STRUMPACK_MC64B/BD (IWAY = 1) or */
/*     STRUMPACK_MC64W/WD (IWAY = 2) */
...
/* ********************************************************************** */
/* Subroutine */ int_t strumpack_mc64fd_(int_t *pos0, int_t *qlen, int_t *n, 
...
/* ********************************************************************** */
/* Subroutine */ int_t strumpack_mc64rd_(int_t *n, int_t *ne, int_t *ip, int_t *
	irn, double *a)
{
    /* System generated locals */
    int_t i__1, i__2, i__3;
...
/* ********************************************************************** */
/* Subroutine */ int_t strumpack_mc64sd_(int_t *n, int_t *ne, int_t *ip, int_t *
	...
	int_t *iw4)
{
    ...
    extern /* Subroutine */ int_t strumpack_mc64qd_(int_t *, int_t *, int_t *, 
	    int_t *, int_t *, double *, int_t *, double *), 
	    strumpack_mc64ud_(int_t *, int_t *, int_t *, int_t *, int_t *, 
	    int_t *, int_t *, int_t *, int_t *, int_t *, int_t *, 
...
/* IW4 is int_t work array of length 4N used by STRUMPACK_MC64U/UD. */
/*      EXTERNAL FD05AD,STRUMPACK_MC64QD,STRUMPACK_MC64UD */
/*      DOUBLE PRECISION FD05AD */
...
/* CNT is the number of calls made to STRUMPACK_MC64U/UD so far. */
...
/* The first call to STRUMPACK_MC64U/UD */
    cnt = 1;
    mod = 1;
    *numx = 0;
    strumpack_mc64ud_(&cnt, &mod, n, &irn[1], ne, &ip[1], &len[1], &fc[1], &iw[1], numx,
...
/* Find splitting value BVAL */
		strumpack_mc64qd_(&ip[1], &lenl[1], &len[1], &w[1], &wlen, &a[1], &nval,
			 &bval);
		if (nval <= 1) {
		    goto L99;
		}
...
/* End of dummy loop; this point is never reached */
/* Set mode for next call to STRUMPACK_MC64U/UD */
L81:
	    mod = 1;
	} else {
...
/* Find splitting value BVAL */
	    strumpack_mc64qd_(&ip[1], &len[1], &lenh[1], &w[1], &wlen, &a[1], &nval, &
		    bval);
	    if (nval == 0 || bval == bmin) {
		goto L99;
	    }
...
L88:
	...
	++cnt;
	strumpack_mc64ud_(&cnt, &mod, n, &irn[1], ne, &ip[1], &len[1], &fc[1], &iw[1], &
...
/* ********************************************************************** */
/* Subroutine */ int_t strumpack_mc64qd_(int_t *ip, int_t *lenl, int_t *lenh, 
...
/* ********************************************************************** */
/* Subroutine */ int_t strumpack_mc64ud_(int_t *id, int_t *mod, int_t *n, int_t *
...
/* Not the first call to STRUMPACK_MC64U/UD. */
/* Re-initialize ARP if entries were deleted since last call to STRUMPACK_MC64U/UD */
	if (*mod == 1) {
	    i__1 = *n;
	    for (i__ = 1; i__ <= i__1; ++i__) {
		arp[i__] = 0;
...
/* NFC is number of rows/columns that could not be assigned */
    nfc = 0;
/* Integers ID0+1 to ID0+N are unique numbers for call ID to STRUMPACK_MC64U/UD, */
...
/* ********************************************************************** */
/* Subroutine */ int_t strumpack_mc64wd_(int_t *n, int_t *ne, int_t *ip, int_t *
	irn, double *a, int_t *iperm, int_t *num, int_t *jperm, 
...
/* Local parameters */
/* External subroutines and/or functions */
/*      EXTERNAL FD05AD,STRUMPACK_MC64DD,STRUMPACK_MC64ED,STRUMPACK_MC64FD */
/*      DOUBLE PRECISION FD05AD */
/* Set RINF to largest positive real number */
/* XSL      RINF = FD05AD(5) */
    /* Parameter adjustments */
...
/* Initialize heap Q and Q2 with rows held in Q(1:QLEN) */
	...
	for (kk = 1; kk <= i__2; ++kk) {
		...
		++qlen;
		l[i__] = qlen;
		strumpack_mc64dd_(&i__, n, &q[1], &d__[1], &l[1], &c__2);
	    }
/* Update tree */
	    jj = iperm[i__];
	    out[jj] = k;
...
/* If Q2 is empty, extract rows from Q */
		...
		dmin__ = d__[i__];
L152:
		strumpack_mc64ed_(&qlen, n, &q[1], &d__[1], &l[1], &c__2);
		--low;
		q[low] = i__;
		l[i__] = low;
		if (qlen == 0) {
