...
namespace strumpack {
  namespace gpu {
    ...
    gemm(BLASHandle& handle, cublasOperation_t ta, cublasOperation_t tb,
      ...
      gemm(handle, ta, tb, c.rows(), c.cols(),
           (ta==CUBLAS_OP_N) ? a.cols() : a.rows(), alpha, a.data(), a.ld(),
           b.data(), b.ld(), beta, c.data(), c.ld());
    }
    template void gemm(BLASHandle&, cublasOperation_t, cublasOperation_t,
    ...
    getrs(SOLVERHandle& handle, cublasOperation_t trans,
      ...
      getrs(handle, trans, A.rows(), B.cols(), A.data(), A.ld(),
            devIpiv, B.data(), B.ld(), devInfo);
    }
    template void getrs(SOLVERHandle&, cublasOperation_t,
                        const DenseMatrix<float>&, const int*,
                        DenseMatrix<float>&, int*);
    template void getrs(SOLVERHandle&, cublasOperation_t,
                        const DenseMatrix<double>&, const int*,
