      SUBROUTINE PDGEQPFHMT( M, N, A, IA, JA, DESCA, IPIV, TAU, WORK,
     $     LWORK, INFO, JPERM, JPIV, RANK, RTOL, ATOL, MINMN)
*
...
*
      IMPLICIT NONE
*     .. Scalar Arguments ..
      INTEGER            IA, JA, INFO, LWORK, M, N, RANK, MINMN
*     ..
*     .. Array Arguments ..
...
*     .. Local Scalars ..
     ...
     $                   MYCOL, MYROW, NPCOL, NPROW, NQ, NQ0, PVT
      DOUBLE PRECISION   AJJ, ALPHA, TEMP, TEMP2, A11, HMTSCAL
*     ..
*     .. Local Arrays ..
...
*
     ...
     $                    -1, -1, -1)
            HMTSCAL = 1. + 4. * SQRT(REAL(M) * MINMN) / (M-J+JA-1)
            IF(J.EQ.JA) THEN
              IF(ABS(TEMP)<ATOL/HMTSCAL) THEN
                GOTO 99
              END IF
              A11=ABS(TEMP)
            ELSE
               IF(ABS(TEMP)/A11<RTOL/HMTSCAL .OR.
     $              ABS(TEMP)<ATOL/HMTSCAL) THEN
                  GOTO 99
               END IF
            END IF
         ELSE
