...
namespace strumpack {
  ...
  (integer_t n, SCOTCH_Graph* graph, SCOTCH_Strat* strat, integer_t* permtab, integer_t* peritab,
   integer_t* cblkptr, SCOTCH_Num* rangtab, SCOTCH_Num* treetab) {
    ...
    int ierr = SCOTCH_graphOrder(graph, strat, scotch_permtab, scotch_peritab,
  ...
  (SCOTCH_Num n, SCOTCH_Graph* graph, SCOTCH_Strat* strat, SCOTCH_Num* permtab, SCOTCH_Num* peritab,
   SCOTCH_Num* cblkptr, SCOTCH_Num* rangtab, SCOTCH_Num* treetab) {
    return SCOTCH_graphOrder(graph, strat, permtab, peritab, cblkptr, rangtab, treetab);
  }
  ...
  template<typename integer_t> std::unique_ptr<SeparatorTree<integer_t>> sep_tree_from_scotch_tree
  (std::vector<SCOTCH_Num>& scotch_tree, std::vector<SCOTCH_Num>& scotch_sizes) {
    // if the graph is disconnected, add one root to connect all
    if (std::count(scotch_tree.begin(), scotch_tree.end(), -1) > 1) {
      SCOTCH_Num root_id = *std::max_element(scotch_tree.begin(), scotch_tree.end()) + 1;
      scotch_tree.push_back(root_id);
      scotch_sizes.push_back(scotch_sizes.back());
      std::replace(scotch_tree.begin(), scotch_tree.end(), SCOTCH_Num(-1), root_id);
    }
	...
	count[p]++;
	switch (count[p]) {
	case 1: scotch_lch[p] = i; break; // node i is the first child of node p
	case 2: scotch_rch[p] = i; break; // node i is the second child of node p
    ...
    std::unique_ptr<SeparatorTree<integer_t>> sep_tree(new SeparatorTree<integer_t>(nbsep));
    ...
    integer_t root = std::distance(scotch_tree.begin(), std::find(scotch_tree.begin(), scotch_tree.end()+nbsep, integer_t(-1)));
    sep_tree->pa()[root] = -1;
    return sep_tree;
  }
  ...
  scotch_nested_dissection(CompressedSparseMatrix<scalar_t,integer_t>* A,
    ...
    if (nnz_nodiag==0) std::cerr << "# WARNING: matrix seems to be diagonal!" << std::endl;
    int ierr = SCOTCH_graphBuild(&graph, 0, n, ptr_nodiag, NULL, NULL, NULL,
  				 ptr_nodiag[n], ind_nodiag, NULL);
    if (ierr) std::cerr << "# ERROR: Scotch failed to build graph." << std::endl;
    assert(SCOTCH_graphCheck(&graph) == 0);

    SCOTCH_Strat strategy;
    ierr = SCOTCH_stratInit(&strategy);
    if (ierr) std::cerr << "# ERROR: Scotch failed to initialize strategy." << std::endl;
    ierr = SCOTCH_stratGraphOrder(&strategy, get_scotch_strategy_string(opts.nd_param()).c_str());
    if (ierr) std::cerr << "# ERROR: Scotch failed to compute ordering." << std::endl;

    std::vector<SCOTCH_Num> scotch_sizes(n+1);
    std::vector<SCOTCH_Num> scotch_tree(n);
    integer_t nbsep;
    ierr = WRAPPER_SCOTCH_graphOrder<integer_t>(n, &graph, &strategy, perm, iperm,
