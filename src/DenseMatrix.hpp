...
namespace strumpack {
  ...
  public:
    ...
    void extract_rows(const std::vector<std::size_t>& I, const DenseMatrix<scalar_t>& B);
    ...
    DenseMatrix<scalar_t>& scatter_rows_add(const std::vector<std::size_t>& I, const DenseMatrix<scalar_t>& B);
    DenseMatrix<scalar_t>& add(const DenseMatrix<scalar_t>& B);
    DenseMatrix<scalar_t>& sub(const DenseMatrix<scalar_t>& B);
    DenseMatrix<scalar_t>& scaled_add(scalar_t alpha, const DenseMatrix<scalar_t>& B);
    real_t norm() const;
    real_t normF() const;
    real_t norm1() const;
    real_t normI() const;
    virtual std::size_t memory() const { return sizeof(scalar_t)*rows()*cols(); }
  ...
  private:
    void ID_column_MGS(DenseMatrix<scalar_t>& X, std::vector<int>& piv, std::vector<std::size_t>& ind,
		       real_t rel_tol, real_t abs_tol, int max_rank, int depth);
    void ID_column_GEQP3(DenseMatrix<scalar_t>& X, std::vector<int>& piv, std::vector<std::size_t>& ind,
			 real_t rel_tol, real_t abs_tol, int max_rank, int depth);
  ...
  public:
    ...
    { this->_data = D; this->_rows = m; this->_cols = n; this->_ld = std::max(std::size_t(1), ld); }
    ...
    void clear() { this->_rows = 0; this->_cols = 0; this->_ld = 1; this->_data = nullptr; }
    ...
    DenseMatrixWrapper<scalar_t>& operator=(const DenseMatrixWrapper<scalar_t>& D)
    { this->_data = D.data(); this->_rows = D.rows(); this->_cols = D.cols(); this->_ld = D.ld(); return *this; }
  ...
  template<typename scalar_t> std::unique_ptr<const DenseMatrixWrapper<scalar_t>>
  ConstDenseMatrixWrapperPtr(std::size_t m, std::size_t n, const scalar_t* D, std::size_t ld) {
    return std::unique_ptr<const DenseMatrixWrapper<scalar_t>>(new DenseMatrixWrapper<scalar_t>(m, n, const_cast<scalar_t*>(D), ld));
  }
  template<typename scalar_t> std::unique_ptr<const DenseMatrixWrapper<scalar_t>>
  ConstDenseMatrixWrapperPtr(std::size_t m, std::size_t n, const DenseMatrix<scalar_t>& D, std::size_t i, std::size_t j) {
    return std::unique_ptr<const DenseMatrixWrapper<scalar_t>>(new DenseMatrixWrapper<scalar_t>(m, n, const_cast<DenseMatrix<scalar_t>&>(D), i, j));
  }


  /** copy submatrix of a at ia,ja of size m,n into b at position ib,jb */
  template<typename scalar_t> void copy
  (std::size_t m, std::size_t n, const DenseMatrix<scalar_t>& a, std::size_t ia, std::size_t ja,
  ...
  template<typename scalar_t> void copy
  (const DenseMatrix<scalar_t>& a, DenseMatrix<scalar_t>& b, std::size_t ib, std::size_t jb) {
    copy(a.rows(), a.cols(), a, 0, 0, b, ib, jb);
  }

  /** copy matrix a into matrix b at position ib, jb */
  template<typename scalar_t> void copy(const DenseMatrix<scalar_t>& a, scalar_t* b, std::size_t ldb) {
  ...
  template<typename scalar_t> DenseMatrix<scalar_t> eye(std::size_t m, std::size_t n) {
  ...
  template<typename scalar_t> DenseMatrix<scalar_t>::DenseMatrix(std::size_t m, std::size_t n)
    : _data(new scalar_t[m*n]), _rows(m), _cols(n), _ld(std::max(std::size_t(1), m)) { }

  template<typename scalar_t>
  DenseMatrix<scalar_t>::DenseMatrix(std::size_t m, std::size_t n, const scalar_t* D, std::size_t ld)
    : _data(new scalar_t[m*n]), _rows(m), _cols(n), _ld(std::max(std::size_t(1), m)) {
    assert(ld >= m);
  ...
  template<typename scalar_t> DenseMatrix<scalar_t>::DenseMatrix
  (std::size_t m, std::size_t n, const DenseMatrix<scalar_t>& D, std::size_t i, std::size_t j)
    : _data(new scalar_t[m*n]), _rows(m), _cols(n), _ld(std::max(std::size_t(1), m)) {
  ...
  template<typename scalar_t> DenseMatrix<scalar_t>::DenseMatrix(const DenseMatrix<scalar_t>& D)
    : _data(new scalar_t[D.rows()*D.cols()]), _rows(D.rows()), _cols(D.cols()),
      _ld(std::max(std::size_t(1), D.rows())) {
    for (std::size_t j=0; j<_cols; j++)
      for (std::size_t i=0; i<_rows; i++)
	operator()(i, j) = D(i, j);
  }

  template<typename scalar_t> DenseMatrix<scalar_t>::DenseMatrix(DenseMatrix<scalar_t>&& D)
  ...
  template<typename scalar_t> void DenseMatrix<scalar_t>::print(std::string name, bool all, int width) const {
  ...
  DenseMatrix<scalar_t>::print_to_file(std::string name, std::string filename, int width) const {
  ...
  template<typename scalar_t> void
  DenseMatrix<scalar_t>::random(random::RandomGeneratorBase<typename RealType<scalar_t>::value_type>& rgen) {
  ...
  template<typename scalar_t> void DenseMatrix<scalar_t>::resize(std::size_t m, std::size_t n) {
  ...
  template<typename scalar_t> void DenseMatrix<scalar_t>::copy(const scalar_t* B, std::size_t ldb) {
    assert(ldb >= rows());
    for (std::size_t j=0; j<cols(); j++)
      for (std::size_t i=0; i<rows(); i++)
  ...
  template<typename scalar_t> DenseMatrix<scalar_t> DenseMatrix<scalar_t>::transpose() const {
    DenseMatrix<scalar_t> tmp(cols(), rows());
    blas::omatcopy('C', rows(), cols(), const_cast<scalar_t*>(data()), ld(), tmp.data(), tmp.ld());
    return tmp;
  }

  template<typename scalar_t> void DenseMatrix<scalar_t>::permute_rows_bwd(const std::vector<int>& P) {
    blas::laswp(cols(), data(), ld(), 1, P.size(), const_cast<int*>(P.data()), -1);
  }

  template<typename scalar_t> void DenseMatrix<scalar_t>::permute_rows_fwd(const std::vector<int>& P) {
    blas::laswp(cols(), data(), ld(), 1, P.size(), const_cast<int*>(P.data()), 1);
  }
  ...
  template<typename scalar_t> DenseMatrix<scalar_t> DenseMatrix<scalar_t>::extract_rows
  ...
  template<typename scalar_t> DenseMatrix<scalar_t> DenseMatrix<scalar_t>::extract
  ...
  template<typename scalar_t> DenseMatrix<scalar_t>& DenseMatrix<scalar_t>::add(const DenseMatrix<scalar_t>& B) {
  ...
  template<typename scalar_t> DenseMatrix<scalar_t>& DenseMatrix<scalar_t>::sub(const DenseMatrix<scalar_t>& B) {
    //#pragma omp taskloop default(shared) //grainsize(128) //collapse(2)
    for (std::size_t j=0; j<cols(); j++)
      for (std::size_t i=0; i<rows(); i++)
	operator()(i, j) -= B(i, j);
  ...
  template<typename scalar_t> typename RealType<scalar_t>::value_type DenseMatrix<scalar_t>::norm() const {
    return normF();
  }

  template<typename scalar_t> typename RealType<scalar_t>::value_type DenseMatrix<scalar_t>::norm1() const {
    return blas::lange<scalar_t,real_t>('1', rows(), cols(), const_cast<scalar_t*>(data()), ld());
  }

  template<typename scalar_t> typename RealType<scalar_t>::value_type DenseMatrix<scalar_t>::normI() const {
    return blas::lange<scalar_t,real_t>('I', rows(), cols(), const_cast<scalar_t*>(data()), ld());
  }

  template<typename scalar_t> typename RealType<scalar_t>::value_type DenseMatrix<scalar_t>::normF() const {
    return blas::lange<scalar_t,real_t>('F', rows(), cols(), const_cast<scalar_t*>(data()), ld());
  }

  template<typename scalar_t> std::vector<int> DenseMatrix<scalar_t>::LU(int depth) {
    std::vector<int> piv(rows());
    int info = 0;
    getrf_omp_task(rows(), cols(), data(), ld(), piv.data(), &info, depth);
    if (info) {
      std::cerr << "ERROR: LU factorization failed with info=" << info << std::endl;
      exit(1);
    }
    return piv;
  }
  ...
  template<typename scalar_t> DenseMatrix<scalar_t> DenseMatrix<scalar_t>::solve
  (const DenseMatrix<scalar_t>& b, const std::vector<int>& piv, int depth) const {
    int info = 0;
    DenseMatrix<scalar_t> x(b);
    getrs_omp_task(char(Trans::N), rows(), b.cols(), const_cast<scalar_t*>(data()), ld(),
		   const_cast<int*>(piv.data()), x.data(), x.ld(), &info, depth);
    if (info) {
  ...
  (DenseMatrix<scalar_t>& L, DenseMatrix<scalar_t>& Q, int depth) const {
    ...
    blas::gelqfmod(rows(), cols(), tmp.data(), tmp.ld(), tau, &info, depth); // perform LQ on Q
    if (info) {
      std::cerr << "ERROR: LQ factorization failed with info=" << info << std::endl;
      exit(1);
    }
    ...
    blas::xxglqmod(cols(), cols(), std::min(rows(), cols()), tmp.data(), tmp.ld(), tau, &info, depth);
    Q = DenseMatrix<scalar_t>(cols(), cols(), tmp, 0, 0); // generate Q
    if (info) {
      std::cerr << "ERROR: generation of Q from LQ failed with info=" << info << std::endl;
      exit(1);
    }
    delete[] tau;
  }
  ...
  (DenseMatrix<scalar_t>& X, std::vector<int>& piv, std::vector<std::size_t>& ind,
   real_t rel_tol, real_t abs_tol, int max_rank, int depth) const {
  ...
  (DenseMatrix<scalar_t>& X, std::vector<int>& piv, std::vector<std::size_t>& ind,
   real_t rel_tol, real_t abs_tol, int max_rank, int depth) {
    ...
    blas::geqp3tol(m, n, data(), ld(), iind.data(), tau, &info, rank, rel_tol, abs_tol, depth);
    ...
    trsm_omp_task('L', 'U', 'N', 'N', rank, n-rank, scalar_t(1.), data(), ld(), ptr(0, rank), ld(), depth);
  ...
  (DenseMatrix<scalar_t>& X, std::vector<int>& piv, std::vector<std::size_t>& ind,
   real_t rel_tol, real_t abs_tol, int max_rank, int depth) {
    ...
    //#pragma omp taskloop grainsize(32) default(shared) if(depth < params::task_recursion_cutoff_level) final(depth >= params::task_recursion_cutoff_level-1) mergeable
	...
	if (i < n-1) {
	  gemv_omp_task('C', m, n-(i+1), scalar_t(1.), ptr(0, i+1), ld(),
			ptr(0, i), 1, scalar_t(0.), R.ptr(i, i+1), R.ld(), depth);
      ...
      if (i < n-1)
	gemm_omp_task('N', 'N', m, n-(i+1), i-j, scalar_t(-1.), ptr(0, j), ld(),
		      R.ptr(j, i+1), R.ld(), scalar_t(1.), ptr(0, i+1), ld(), depth);
    }
    delete[] cnrms;
    ind.resize(i);
    ...
    std::cout << "TODO count flops for ID_column_MGS, note that this routine is not stable!" << std::endl;
  }


  template<typename scalar_t> void
  gemm(Trans ta, Trans tb, scalar_t alpha, const DenseMatrix<scalar_t>& a, const DenseMatrix<scalar_t>& b,
    ...
    assert((ta==Trans::N && tb==Trans::N && a.cols()==b.rows()) || (ta!=Trans::N && tb==Trans::N && a.rows()==b.rows()) ||
	   (ta==Trans::N && tb!=Trans::N && a.cols()==b.cols()) || (ta!=Trans::N && tb!=Trans::N && a.rows()==b.cols()));
    gemm_omp_task(char(ta), char(tb), c.rows(), c.cols(), (ta==Trans::N) ? a.cols() : a.rows(),
		  alpha, const_cast<scalar_t*>(a.data()), a.ld(), const_cast<scalar_t*>(b.data()), b.ld(),
		  beta, c.data(), c.ld(), depth);
  }

  template<typename scalar_t> void
  gemm(Trans ta, Trans tb, scalar_t alpha, const DenseMatrix<scalar_t>& a, const scalar_t* b, int ldb,
       scalar_t beta, DenseMatrix<scalar_t>& c, int depth=0) {
    assert((ta==Trans::N && a.rows()==c.rows()) || (ta!=Trans::N && a.cols()==c.rows()));
    gemm_omp_task(char(ta), char(tb), c.rows(), c.cols(), (ta==Trans::N) ? a.cols() : a.rows(),
		  alpha, const_cast<scalar_t*>(a.data()), a.ld(),
		  const_cast<scalar_t*>(b), ldb, beta, c.data(), c.ld(), depth);
  }

  template<typename scalar_t> void
  gemm(Trans ta, Trans tb, scalar_t alpha, const DenseMatrix<scalar_t>& a, const DenseMatrix<scalar_t>& b,
       scalar_t beta, scalar_t* c, int ldc, int depth=0) {
    assert((ta==Trans::N && tb==Trans::N && a.cols()==b.rows()) || (ta!=Trans::N && tb==Trans::N && a.rows()==b.rows()) ||
	   (ta==Trans::N && tb!=Trans::N && a.cols()==b.cols()) || (ta!=Trans::N && tb!=Trans::N && a.rows()==b.cols()));
    gemm_omp_task(char(ta), char(tb), (ta==Trans::N) ? a.rows() : a.cols(), (tb==Trans::N) ? b.cols() : b.rows(),
		  (ta==Trans::N) ? a.cols() : a.rows(), alpha, const_cast<scalar_t*>(a.data()), a.ld(),
		  const_cast<scalar_t*>(b.data()), b.ld(), beta, c, ldc, depth);
  }
   ...
   */
  template<typename scalar_t> void
  trmm(Side s, UpLo ul, Trans ta, Diag d, scalar_t alpha, const DenseMatrix<scalar_t>& a,
       DenseMatrix<scalar_t>& b, int depth) {
    trmm_omp_task(char(s), char(ul), char(ta), char(d), b.rows(), b.cols(), alpha,
		  const_cast<scalar_t*>(a.data()), a.ld(), b.data(), b.ld(), depth);
  }
   ...
   */
  template<typename scalar_t> void
  trsm(Side s, UpLo ul, Trans ta, Diag d, scalar_t alpha, const DenseMatrix<scalar_t>& a,
       DenseMatrix<scalar_t>& b, int depth) {
    // TODO assertions
    trsm_omp_task(char(s), char(ul), char(ta), char(d), b.rows(), b.cols(), alpha,
		  const_cast<scalar_t*>(a.data()), a.ld(), b.data(), b.ld(), depth);
  }
  ...
  trsv(UpLo ul, Trans ta, Diag d, const DenseMatrix<scalar_t>& a,
    ...
    trsv_omp_task(char(ul), char(ta), char(d), a.rows(),
		  const_cast<scalar_t*>(a.data()), a.ld(), b.data(), 1, depth);
  }
  ...
  gemv(Trans ta, scalar_t alpha, const DenseMatrix<scalar_t>& a,
    ...
    gemv_omp_task(char(ta), a.rows(), a.cols(), alpha, const_cast<scalar_t*>(a.data()), a.ld(),
		  const_cast<scalar_t*>(x.data()), 1, beta, y.data(), 1, depth);
  }
