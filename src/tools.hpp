...
namespace strumpack {
  ...
  x_mult_y(integer_t n, scalar_t* x, scalar_t* y, scalar_t* z) {
#pragma omp parallel for
    for (int i=0; i<n; ++i) z[i] = x[i] * y[i];
    STRUMPACK_FLOPS((is_complex<scalar_t>()?6:1)*static_cast<long long int>(n));
    STRUMPACK_BYTES(sizeof(scalar_t)*static_cast<long long int>(n)*4);
  }

  /** compute x[i] *= y[i] for two vectors with n elements */
  template<typename scalar_t,typename integer_t> inline void
  x_mult_y(integer_t n, scalar_t* x, scalar_t* y) {
#pragma omp parallel for
  ...
  /** compute x[i] /= y[i] for two vectors with n elements */
  template<typename scalar_t,typename integer_t> inline void
  x_div_y(integer_t n, scalar_t* x, scalar_t* y) {
#pragma omp parallel for
    for (int i=0; i<n; ++i) x[i] /= y[i];
    STRUMPACK_FLOPS((is_complex<scalar_t>()?11:1)*static_cast<long long int>(n));
    STRUMPACK_BYTES(sizeof(scalar_t)*static_cast<long long int>(n)*3);
  }

  template<typename scalar_t,typename integer_t> inline void
  mat_print(std::string name, scalar_t* F, integer_t rows, integer_t cols, integer_t ld) {
  ...
  // b = a + beta * b
  template<typename scalar_t,typename integer_t> inline void
  mat_add(integer_t m, integer_t n, scalar_t* a, integer_t lda, scalar_t beta, scalar_t* b, integer_t ldb) {
    if (beta == scalar_t(1.)) {
  ...
  template<typename scalar_t,typename integer_t> inline void
  permute_vector(integer_t n, scalar_t* vect, integer_t* order, bool forward) {
    scalar_t* tmp = new scalar_t[n];
    if (forward) {
#pragma omp parallel for
      for (integer_t k=0; k<n; k++) tmp[k] = vect[order[k]];
    } else {
#pragma omp parallel for
      for (integer_t k=0; k<n; k++) tmp[order[k]] = vect[k]; // I don't think parallel for helps here
    }
  ...
  template<typename scalar_t,typename integer_t> inline void
  permute_vector(integer_t n, scalar_t* vect, std::vector<integer_t>& order, bool forward) {
    permute_vector(n, vect, order.data(), forward);
  }

  template<typename integer_t> inline void
  merge_indices(integer_t* v1, integer_t& l1, integer_t l1max, integer_t* v2, integer_t l2) {
    integer_t* result = v1;
  ...
  // inline void merge_indices(int *v1, int& v1length, int l1max, int *v2, int v2length) {
  //   int cntv2 = 0;
  //   int cntv1 = 0;
  //   while (cntv1<v1length && cntv2<v2length) {
  //     if (v1[cntv1] > v2[cntv2]) {
  //       for (int i=v1length; i>cntv1; i--)
  // 	v1[i] = v1[i-1];      // shift V1
  //       v1[cntv1] = v2[cntv2];  // insert V2[cntv2]
  //       v1length++;             // v1 becomes longer
  //       cntv2++;
  //     } else
  //       if (v1[cntv1] == v2[cntv2])
  // 	cntv2++;
  //     cntv1++;
  //   }
  //   for (int i=cntv2; i<v2length; i++)
  //     v1[v1length++] = v2[i];
  // }

  // this sorts both indices and values at the same time
  // by first copying everything to a vector of std::pair objects,
  // then sorting that and then copying back
  // template<typename scalar>
  // inline void sort_indices_values(int *ind, scalar *val, int begin, int end) {
  //   std::vector<std::pair<int,scalar>> l(end-begin);
  //   for (int k=begin; k<end; k++)
  //     l[k-begin] = std::make_pair(ind[k], val[k]);
  //   std::sort(l.begin(), l.end(), [](const std::pair<int,scalar>& a, const std::pair<int,scalar>& b) -> bool {
  //       // sort based on the column,row indices
  //       return std::get<0>(a) < std::get<0>(b); });
  //   for (int k=begin; k<end; k++) {
  //     ind[k] = std::get<0>(l[k-begin]);
  //     val[k] = std::get<1>(l[k-begin]);
  //   }
  // }

  // this sorts both indices and values at the same time
  template<typename scalar_t,typename integer_t> void
  sort_indices_values(integer_t *ind, scalar_t *val, integer_t begin, integer_t end) {
    if (end > begin) {
