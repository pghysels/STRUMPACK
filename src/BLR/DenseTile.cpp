...
namespace strumpack {
  namespace BLR {
    ...
    template<typename scalar_t> void
    DenseTile<scalar_t>::move_to_gpu(scalar_t*& dptr, scalar_t* pinned) {
      ...
      if (!pinned) {
        ...
        hD.copy(D());
        gpu_check(gpu::copy_host_to_device(dD, hD));
      }
