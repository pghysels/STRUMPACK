...
namespace strumpack {
  ...
  /**
   * Permute a vector which is distributed over a number of processes according to dist.
  ...
  permute_vector(scalar_t* x, std::vector<integer_t>& iorder, std::vector<integer_t>& dist, MPI_Comm comm) {
    ...
    for (integer_t r=0; r<n; r++) {
      dest[r] = std::upper_bound(dist.begin(), dist.end(), iorder[r+lo]) - dist.begin() - 1;
      scnts[dest[r]]++;
    }
    sdispls[0] = 0;
    pp[0] = sbuf;
    ...
    MPI_Alltoallv(sbuf, scnts, sdispls, MPI_BYTE, rbuf, rcnts, rdispls, MPI_BYTE, comm);
    delete[] dest;
    delete[] sbuf;
#pragma omp parallel for
  ...
  public:
    ...
    (MatrixReorderingMPI<scalar_t,integer_t>* nd, std::vector<integer_t>* _upd, float* _work,
     integer_t P0, integer_t P, integer_t P0_sibling, integer_t P_sibling, integer_t owner, MPI_Comm comm) {
      auto rank = mpi_rank(comm);
	...
	size_t msg_size = sizeof(integer_t)*(1+nbsep*3+1+1) + sizeof(float)*nbsep;
	for (integer_t i=0; i<nbsep; i++)
	  msg_size += sizeof(integer_t)*(1+_upd[i].size());
	std::vector<int> hss_partitions = serialize(nd->local_sep_tree->HSS_trees());
	...
	*reinterpret_cast<integer_t*>(p_buf) = nbsep;
	p_buf += sizeof(nbsep);
	std::copy(nd->local_sep_tree->lch(), nd->local_sep_tree->lch()+nbsep, reinterpret_cast<integer_t*>(p_buf));
	p_buf += sizeof(integer_t)*nbsep;
	std::copy(nd->local_sep_tree->rch(), nd->local_sep_tree->rch()+nbsep, reinterpret_cast<integer_t*>(p_buf));
	p_buf += sizeof(integer_t)*nbsep;
	...
	*reinterpret_cast<integer_t*>(p_buf) = nd->local_sep_tree->root();
	...
	for (integer_t s=0; s<nbsep+1; s++) {
	  *reinterpret_cast<integer_t*>(p_buf) = nd->local_sep_tree->sizes()[s] + nd->sub_graph_range.first;
	  p_buf += sizeof(integer_t);
	}
	for (integer_t i=0; i<nbsep; i++) {
	  *reinterpret_cast<integer_t*>(p_buf) = static_cast<integer_t>(_upd[i].size());
	  p_buf += sizeof(integer_t);
	}
	for (integer_t i=0; i<nbsep; i++) {
	  std::copy(_upd[i].begin(), _upd[i].end(), reinterpret_cast<integer_t*>(p_buf));
	  p_buf += sizeof(integer_t)*_upd[i].size();
	}
	std::copy(hss_partitions.begin(), hss_partitions.end(), reinterpret_cast<int*>(p_buf));
	...
	if (msg_size >= std::numeric_limits<int>::max())
	  std::cerr << "ERROR: In " << __FILE__ << ", line " << __LINE__ << ",\n"
		    << "\tmessage is more than " << std::numeric_limits<int>::max() << " bytes." << std::endl
		    << "\tPlease send this message to the STRUMPACK developers." << std::endl;
	send_req.resize(dest1-dest0);

	// TODO the sibling only needs the root of the tree!!
	for (int dest=dest0; dest<dest1; dest++)
	  MPI_Isend(send_buf, msg_size, MPI_BYTE, dest, 0, comm, &send_req[dest-dest0]);
      }
	...
	send_req.clear();
      ...
      if (receiver) {
	char* p_data = data;
	nr_sep = *((integer_t*)p_data);  p_data += sizeof(integer_t);
	lchild = (integer_t*)p_data;     p_data += nr_sep * sizeof(integer_t);
	rchild = (integer_t*)p_data;     p_data += nr_sep * sizeof(integer_t);
	root = *((integer_t*)p_data);    p_data += sizeof(integer_t);
	work = (float*)p_data;           p_data += nr_sep * sizeof(float);
	sep_ptr = (integer_t*)p_data;    p_data += (nr_sep+1) * sizeof(integer_t);
	dim_upd = (integer_t*)p_data;    p_data += nr_sep * sizeof(integer_t);
	...
	int sep_tree_size = *((int*)p_data);                             p_data += sizeof(int);
	for (int t=0; t<sep_tree_size; t++) {
	  int sep = *((int*)p_data);                                     p_data += sizeof(int);
	  int buf_size = *((int*)p_data);                                p_data += sizeof(int);
	  sep_HSS_tree[sep] = HSS::HSSPartitionTree(buf_size, (int*)p_data);  p_data += buf_size * sizeof(int);
	}
      }
    }
