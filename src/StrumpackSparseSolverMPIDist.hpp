...
namespace strumpack {
  ...
  class StrumpackSparseSolverMPIDist : public StrumpackSparseSolverMPI<scalar_t,integer_t> {
  public:
     ...
     */
    StrumpackSparseSolverMPIDist(MPI_Comm mpi_comm, int argc, char* argv[], bool verbose=true);
     ...
     */
    void set_distributed_csr_matrix(integer_t local_rows, integer_t* row_ptr, integer_t* col_ind,
				    scalar_t* values, integer_t* dist, bool symmetric_pattern=false);
    /*! \brief Associate a (PETSc) MPIAIJ block-row distributed CSR matrix with the solver object. */
    void set_MPIAIJ_matrix(integer_t local_rows, integer_t* d_ptr, integer_t* d_ind, scalar_t* d_val,
  ...
  protected:
    virtual CompressedSparseMatrix<scalar_t,integer_t>* matrix() { return _mat_mpi; }
    virtual MatrixReordering<scalar_t,integer_t>* reordering() { return _nd_mpi; }
    virtual EliminationTree<scalar_t,integer_t>* elimination_tree() { return _et_mpi_dist; }
  ...
  template<typename scalar_t,typename integer_t>
  StrumpackSparseSolverMPIDist<scalar_t,integer_t>::StrumpackSparseSolverMPIDist
  (MPI_Comm mpi_comm, bool verbose) :
    StrumpackSparseSolverMPIDist<scalar_t,integer_t>(mpi_comm, 0, nullptr, verbose) {
  }

  template<typename scalar_t,typename integer_t>
  StrumpackSparseSolverMPIDist<scalar_t,integer_t>::StrumpackSparseSolverMPIDist
  (MPI_Comm mpi_comm, int argc, char* argv[], bool verbose) :
    StrumpackSparseSolverMPI<scalar_t,integer_t>(mpi_comm, argc, argv, verbose) {
  ...
  (integer_t N, integer_t* row_ptr, integer_t* col_ind,
   scalar_t* values, bool symmetric_pattern) {
    auto mat_seq = new CSRMatrix<scalar_t,integer_t>(N, row_ptr, col_ind, values, symmetric_pattern);
    if (_mat_mpi) delete _mat_mpi;
    _mat_mpi = new CSRMatrixMPI<scalar_t,integer_t>(mat_seq, this->_comm, true);
    delete mat_seq;
    this->_factored = this->_reordered = false;
  }
  ...
  (integer_t local_rows, integer_t* row_ptr, integer_t* col_ind,
   scalar_t* values, integer_t* dist, bool symmetric_pattern) {
    ...
    _mat_mpi = new CSRMatrixMPI<scalar_t,integer_t>
      (local_rows, row_ptr, col_ind, values, dist, this->_comm, symmetric_pattern);
    this->_factored = this->_reordered = false;
  }
  ...
  (integer_t local_rows, integer_t* d_ptr, integer_t* d_ind, scalar_t* d_val,
   integer_t* o_ptr, integer_t* o_ind, scalar_t* o_val, integer_t* garray) {
    if (_mat_mpi) delete _mat_mpi;
    _mat_mpi = new CSRMatrixMPI<scalar_t,integer_t>
      (local_rows, d_ptr, d_ind, d_val, o_ptr, o_ind, o_val, garray, this->_comm);
    this->_factored = this->_reordered = false;
  }

  template<typename scalar_t,typename integer_t> void
  StrumpackSparseSolverMPIDist<scalar_t,integer_t>::setup_matrix_reordering() {
    if (_nd_mpi) delete _nd_mpi;
    _nd_mpi = new MatrixReorderingMPI<scalar_t,integer_t>(matrix()->size(), this->_comm);
  }

  template<typename scalar_t,typename integer_t> int
  StrumpackSparseSolverMPIDist<scalar_t,integer_t>::compute_reordering(int nx, int ny, int nz) {
    return _nd_mpi->nested_dissection(this->_opts, _mat_mpi, nx, ny, nz);
  }

  template<typename scalar_t,typename integer_t> void
  StrumpackSparseSolverMPIDist<scalar_t,integer_t>::compute_separator_reordering() {
    //return _nd_mpi->separator_reordering(this->_opts, _mat_mpi, _et_mpi_dist->root_front());
    return _nd_mpi->separator_reordering(this->_opts, _mat_mpi, this->_opts.verbose() && this->_is_root);
  }
  ...
  (scalar_t* b, scalar_t* x, bool use_initial_guess) {
    ...
    if (this->_opts.mc64job() == 5) x_mult_y(n_local, b, this->_mc64_Dr.data());

    auto gmres = [&](std::function<void(scalar_t*)> prec) {
      GMResMPI<scalar_t,integer_t>
      (this->_comm, _mat_mpi, prec, n_local, x, b, this->_opts.rel_tol(), this->_opts.abs_tol(),
       this->_Krylov_its, this->_opts.maxit(), this->_opts.gmres_restart(),
       this->_opts.GramSchmidt_type(), use_initial_guess, this->_opts.verbose() && this->_is_root);
    };
    auto bicgstab = [&](std::function<void(scalar_t*)> prec) {
      BiCGStabMPI<scalar_t,integer_t>
      (this->_comm, _mat_mpi, prec, n_local, x, b, this->_opts.rel_tol(), this->_opts.abs_tol(),
       this->_Krylov_its, this->_opts.maxit(), use_initial_guess, this->_opts.verbose() && this->_is_root);
    };
