...
namespace strumpack {
  ...
  public:
    ...
    (DistM_t& CB, DistM_t& F11, DistM_t& F12, DistM_t& F21, DistM_t& F22,
     std::vector<std::vector<scalar_t>>& sbuf, FrontalMatrixDenseMPI<scalar_t,integer_t>* pa,
	...
	for (int c=0; c<lcols; c++) pa_col[c] = I[CB.coll2g(c)];
      ...
      int r_upd, c_upd;
      for (r_upd=0; r_upd<lrows; r_upd++) if (pa_row[r_upd] >= pa->dim_sep) break;
      for (c_upd=0; c_upd<lcols; c_upd++) if (pa_col[c_upd] >= pa->dim_sep) break;
      if (CB.fixed()) {
	for (int c=0; c<c_upd; c++) // F11
	  for (int r=0; r<r_upd; r++)
	    sbuf[pa->find_rank_fixed(pa_row[r], pa_col[c], F11)].push_back(CB(r,c));
	for (int c=c_upd; c<lcols; c++) // F12
	  for (int r=0; r<r_upd; r++)
	    sbuf[pa->find_rank_fixed(pa_row[r], pa_col[c]-pa->dim_sep, F12)].push_back(CB(r,c));
	for (int c=0; c<c_upd; c++) // F21
	  for (int r=r_upd; r<lrows; r++)
	    sbuf[pa->find_rank_fixed(pa_row[r]-pa->dim_sep, pa_col[c], F21)].push_back(CB(r,c));
	for (int c=c_upd; c<lcols; c++) // F22
	  for (int r=r_upd; r<lrows; r++)
	    sbuf[pa->find_rank_fixed(pa_row[r]-pa->dim_sep, pa_col[c]-pa->dim_sep, F22)].push_back(CB(r,c));
      } else {
	for (int c=0; c<c_upd; c++) // F11
	  for (int r=0; r<r_upd; r++)
	    sbuf[pa->find_rank(pa_row[r], pa_col[c], F11)].push_back(CB(r,c));
	for (int c=c_upd; c<lcols; c++) // F12
	  for (int r=0; r<r_upd; r++)
	    sbuf[pa->find_rank(pa_row[r], pa_col[c]-pa->dim_sep, F12)].push_back(CB(r,c));
	for (int c=0; c<c_upd; c++) // F21
	  for (int r=r_upd; r<lrows; r++)
	    sbuf[pa->find_rank(pa_row[r]-pa->dim_sep, pa_col[c], F21)].push_back(CB(r,c));
	for (int c=c_upd; c<lcols; c++) // F22
	  for (int r=r_upd; r<lrows; r++)
	    sbuf[pa->find_rank(pa_row[r]-pa->dim_sep, pa_col[c]-pa->dim_sep, F22)].push_back(CB(r,c));
      }
      delete[] pa_row;
    }
    ...
    (DistM_t& Bchild, DistM_t& Bsep, DistM_t& Bupd, std::vector<std::vector<scalar_t>>& sbuf,
    ...
    (const DistM_t& F, const std::vector<std::size_t>& I, const std::vector<std::size_t>& J,
     const std::vector<std::size_t>& oI, const std::vector<std::size_t>& oJ,
	...
	for (std::size_t c=0; c<J.size(); c++) {
	  ...
	  for (std::size_t r=0; r<I.size(); r++) {
	    ...
	    if (F.rowg2p_fixed(gr) == F.prow())
	      sbuf[B.rank_fixed(oI[r],oJ[c])].push_back(F(F.rowg2l_fixed(gr),lc));
	  }
	}
      } else {
	for (std::size_t c=0; c<J.size(); c++) {
    ...
    (DistM_t& F11, DistM_t& F12, DistM_t& F21, DistM_t& F22, scalar_t** pbuf,
     integer_t sep_begin, integer_t* pa_upd, integer_t* ch_upd, integer_t ch_dim_upd,
     std::function<int(integer_t,integer_t)> F22rank) {
      std::function<integer_t(integer_t)> sep_map = [&](integer_t i) { return i + sep_begin; };
      std::function<integer_t(integer_t)> upd_map = [&](integer_t i) { return pa_upd[i]; };
      copy_from_buffer(F11, pbuf, ch_upd, ch_dim_upd, F22rank, sep_map, sep_map);
      copy_from_buffer(F12, pbuf, ch_upd, ch_dim_upd, F22rank, sep_map, upd_map);
      copy_from_buffer(F21, pbuf, ch_upd, ch_dim_upd, F22rank, upd_map, sep_map);
      copy_from_buffer(F22, pbuf, ch_upd, ch_dim_upd, F22rank, upd_map, upd_map);
    }


    static void skinny_extend_add_copy_to_buffers
    (DistM_t& Schild, DistM_t& S, std::vector<std::vector<scalar_t>>& sbuf,
     FrontalMatrixHSSMPI<scalar_t,integer_t>* pa, std::vector<std::size_t>& I) {
      if (Schild.fixed()) {
      	for (int c=0; c<Schild.lcols(); c++) {
      	  auto gc = Schild.coll2g_fixed(c);
      	  for (int r=0; r<Schild.lrows(); r++)
      	    sbuf[pa->find_rank_fixed(I[Schild.rowl2g_fixed(r)],gc,S)].push_back(Schild(r,c));
      	}
      } else {
	for (int c=0; c<Schild.lcols(); c++) {
	  auto gc = Schild.coll2g(c);
	  for (int r=0; r<Schild.lrows(); r++)
	    sbuf[pa->find_rank(I[Schild.rowl2g(r)],gc,S)].push_back(Schild(r,c));
	}
      }
    }
    ...
    (DistM_t& F, scalar_t** pbuf, integer_t sep_begin, integer_t dim_sep,
     ...
     std::function<int(integer_t,integer_t)> chSrank) {
      if (!F.active()) return;
      auto rowf2g = [&](integer_t r) { return (r < dim_sep) ? r+sep_begin : pa_upd[r-dim_sep]; };
      if (F.fixed()) {
      	for (int c=0; c<F.lcols(); c++) {
    ...
    (DistM_t& Bsep, DistM_t& Bupd, std::vector<std::vector<scalar_t>>& buf,
     ...
     std::function<int(integer_t,integer_t)> b_rank) {
      ...
      std::function<integer_t(integer_t)> sep_map = [&](integer_t i) { return i + sep_begin; };
      std::function<integer_t(integer_t)> upd_map = [&](integer_t i) { return pa_upd[i]; };
      copy_column_from_buffer(Bsep, pbuf, ch_upd, ch_dim_upd, b_rank, sep_map);
      copy_column_from_buffer(Bupd, pbuf, ch_upd, ch_dim_upd, b_rank, upd_map);
    }

    // write a more general skinny_extract / extract_rows
    static void extract_b_copy_from_buffers
    (DistM_t& F, std::vector<std::vector<scalar_t>>& buf, std::vector<std::size_t>& I,
     std::function<int(integer_t)> src_rank) {
      std::vector<scalar_t*> pbuf(buf.size());
      for (size_t p=0; p<buf.size(); p++) pbuf[p] = buf[p].data();
      if (F.fixed())
	for (int r=0; r<F.lrows(); r++)
    ...
    (DistM_t& F, scalar_t** pbuf, integer_t* ch_upd, integer_t ch_dim_upd,
     std::function<int(integer_t,integer_t)> F22rank, std::function<integer_t(integer_t)> fr2g,
     std::function<integer_t(integer_t)> fc2g) {
      if (!F.active()) return;
      // TODO precompute indices??
      //   list of local rows/col that have corresponding global index in front and corresponding child row
      // similar to compute_parent_indices and pass as input because this function is called 4 times
      if (F.fixed()) {
	integer_t upd_c = 0;
	for (int c=0; c<F.lcols(); c++) {
    ...
    (DistM_t& F, std::vector<scalar_t*>& pbuf, integer_t* ch_upd, integer_t ch_dim_upd,
     std::function<int(integer_t,integer_t)> b_child_rank, std::function<integer_t(integer_t)> f2g) {
      if (!F.active()) return;
      integer_t upd_r = 0;
      for (int r=0; r<F.lrows(); r++) {
    ...
    (DistM_t& F, std::vector<std::size_t>& I, std::vector<std::size_t>& J,
     std::vector<std::size_t>& oI, std::vector<std::size_t>& oJ, const DistM_t& B, scalar_t** pbuf) {
      if (!F.active()) return;
      if (F.fixed()) {
    ...
    (DistM_t& F, const std::vector<std::size_t>& oI, const std::vector<std::size_t>& oJ, const DistM_t& B, scalar_t** pbuf) {
      if (!F.active()) return;
      if (F.fixed()) {
	for (std::size_t c=0; c<oJ.size(); c++) {
	  auto gc = oJ[c];
  ...
  template<typename scalar_t,typename integer_t> class ExtractFront {
    ...
    using DistM_t = DistributedMatrix<scalar_t>;
  public:
    static void extract_F11(DistM_t& F, CSM* A, integer_t sep_begin, integer_t dim_sep) {
      if (!F.active()) return;
      F.zero();
...
#pragma omp parallel for collapse(2)
	...
	for (int rb=0; rb<RB; rb++) {
	  ...
	  A->extract_F11_block(block, F.ld(), row+sep_begin, std::min(F.MB(),F.rows()-row), col+sep_begin, nr_cols);
	}
      }
    }

    static void extract_F12(DistM_t& F, CSM* A, integer_t upd_row_begin, integer_t upd_col_begin,
...
#pragma omp parallel for collapse(2)
	...
	for (int rb=0; rb<RB; rb++) {
	  ...
	  auto nr_cols = std::min(F.NB(), F.cols()-col);
	  auto block = F.data() + cb*F.NB()*F.ld() + rb*F.MB();
	  A->extract_F12_block(block, F.ld(), row+upd_row_begin, std::min(F.MB(), F.rows()-row),
			       col+upd_col_begin, nr_cols, block_upd);
	}
      }
    }

    static void extract_F21(DistM_t& F, CSM* A, integer_t upd_row_begin, integer_t upd_col_begin,
			    integer_t dim_upd, integer_t* upd) {
      if (!F.active()) return;
      F.zero();
      const auto CB = F.colblocks();
...
#pragma omp parallel for collapse(2)
	...
	for (int rb=0; rb<RB; rb++) {
	  ...
	  A->extract_F21_block(block, F.ld(), row+upd_row_begin, std::min(F.MB(), F.rows()-row),
			       col+upd_col_begin, nr_cols, block_upd);
	}
      }
    }
