 ...
 */
/**
 * \file SparseSolverMixedPrecisionMPIDist.hpp
 * \brief Contains the mixed precision definition of the
 * distributed sparse solver class.
 */
#ifndef SPARSE_SOLVER_MIXED_PRECISION_MPI_DIST_HPP
#define SPARSE_SOLVER_MIXED_PRECISION_MPI_DIST_HPP
...
namespace strumpack {
  ...
  /**
   * \class SparseSolverMixedPrecisionMPIDist
   *
   * \brief SparseSolverMixedPrecisionMPIDist Allows to use lower
  ...
  public:
     ...
     */
    SparseSolverMixedPrecisionMPIDist(MPI_Comm comm, bool verbose=true);
    ...
    SparseSolverMPIDist<factor_t,integer_t>& solver() { return solver_; }
    const SparseSolverMPIDist<factor_t,integer_t>& solver() const { return solver_; }
  ...
  private:
    CSRMatrixMPI<refine_t,integer_t> mat_;
    SparseSolverMPIDist<factor_t,integer_t> solver_;
    SPOptions<refine_t> opts_;
    int Krylov_its_ = 0;
  };

  template<typename factor_t,typename refine_t,typename integer_t>
  using StrumpackSparseSolverMixedPrecisionMPIDist =
    SparseSolverMixedPrecisionMPIDist<factor_t,refine_t,integer_t>;

} //end namespace strumpack

#endif // STRUMPACK_SPARSE_SOLVER_MIXED_PRECISION_MPI_DIST_HPP
