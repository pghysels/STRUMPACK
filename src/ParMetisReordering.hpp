...
namespace strumpack {
  ...
  (std::vector<integer_t>& dist, idx_t* xadj, idx_t* adjncy, idx_t numflag,
   integer_t* local_order, idx_t* sizes, MPI_Comm c, integer_t local_rows) {
    ...
    int ierr = ParMETIS_V32_NodeND(vtxdist, xadj, adjncy, NULL, &numflag, NULL, NULL, NULL,
  ...
  (std::vector<idx_t>& dist, idx_t* xadj, idx_t* adjncy, idx_t numflag,
   idx_t* local_order, idx_t* sizes, MPI_Comm c, idx_t local_rows) {
    return ParMETIS_V32_NodeND(dist.data(), xadj, adjncy, NULL, &numflag, NULL, NULL, NULL,
  ...
  parmetis_nested_dissection(CSRMatrixMPI<scalar_t,integer_t>* A, MPI_Comm comm,
    ...
    for (integer_t i=0; i<local_rows; i++) { // remove diagonal elements
      ...
      xadj[i+1] = e;
    } // xadj will now point locally, adjncy still has global column/row indices
    idx_t numflag = 0;
    int p2 = std::pow(2, std::floor(std::log2(P)));
    auto local_order = new integer_t[local_rows];
    auto sizes = new idx_t[2*p2-1];
    int ierr = WRAPPER_ParMETIS_V32_NodeND(A->get_dist(), xadj, adjncy, numflag,
  					   local_order, sizes, comm, local_rows);
    if (ierr != METIS_OK) {
      if (!mpi_rank(comm)) std::cerr << "ParMETIS_V32_NodeND failed with ierr=" << ierr << std::endl;
    ...
    if (build_tree) {
      integer_t dist_nr_sep = 2*p2 - 1; // p2 subdomains, p2-1 distributed separators
      sep_tree = std::unique_ptr<SeparatorTree<integer_t>>(new SeparatorTree<integer_t>(dist_nr_sep));
      sep_tree->sizes()[0] = 0;
	...
	sep_tree->rch()[i] = -1;
      ...
      std::function<void(integer_t,integer_t&,integer_t)> build_dist_binary_separator_tree =
	...
	sep_tree->sizes()[pid+1] = sizes[dist_nr_sep-1-dsep] + sep_tree->sizes()[pid];
	pid++;
      };
      integer_t pid = 0;
