...
(int* TRESH, int* NDENSE, int* N, int* IWLEN, int* PE, int* PFREE, int* LEN, int* IW, int* NV, int* ELEN, int* LAST,
 int* NCMPA, int* DEGREE, int* HEAD, int* NEXT, int* W, int* PERM, int* LISTVAR_SCHUR, int* SIZE_SCHUR, int* AGG6);

namespace strumpack {

  template<typename scalar_t,typename integer_t> std::unique_ptr<SeparatorTree<integer_t>>
  aggressive_amalgamation(CompressedSparseMatrix<scalar_t,integer_t>* A,
    ...
    PFREE++;
    MUMPS_SYMQAMD_FC(&TRESH, NDENSE, &N, &IWLEN, PE, &PFREE, LEN, IW, NV, ELEN, LAST, &NCMPA,
		     DEGREE, HEAD, NEXT, W, PERM, LISTVAR_SCHUR, &SIZE_SCHUR, &AGG6);
    for (int i=0; i<N; i++) {
      iperm[i] = LAST[i] - 1;
      perm[i] = ELEN[i] - 1;
    }
    ...
    for (int f=0; f<sep_tree->separators(); f++) sep_tree->lch()[f] = sep_tree->rch()[f] = -1;
    for (int f=0; f<=sep_tree->separators(); f++) sep_tree->sizes()[f] = 0;
    ...
    if (roots > 1) {
      ...
      std::replace(sep_tree->pa(), sep_tree->pa()+fronts, integer_t(-1), integer_t(fronts));
