      SUBROUTINE SLARFBMOD( SIDE, TRANS, DIRECT, STOREV, 
     $     M, N, K, V, LDV,
...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, SGEMM_OMP_TASK, STRMM_OMP_TASK
*     ..
...
*              W := W * V1
*
               CALL STRMM_OMP_TASK( 'Right', 'Lower', 'No transpose',
     $              'Unit', N,
...
*                 W := W + C2**T * V2
*
                  CALL SGEMM_OMP_TASK( 'Transpose', 'No transpose',
     $                 N, K, M-K,
...
*              W := W * T**T  or  W * T
*
               CALL STRMM_OMP_TASK( 'Right', 'Upper', TRANST, 
     $              'Non-unit', N, K,
...
*                 C2 := C2 - V2 * W**T
*
                  CALL SGEMM_OMP_TASK( 'No transpose', 'Transpose',
     $                 M-K, N, K,
...
*              W := W * V1**T
*
               CALL STRMM_OMP_TASK( 'Right', 'Lower', 'Transpose',
     $              'Unit', N, K,
...
*              W := W * V1
*
               CALL STRMM_OMP_TASK( 'Right', 'Lower', 
     $              'No transpose', 'Unit', M,
     $              K, ONE, V, LDV, WORK, LDWORK, DEPTH )
               IF( N.GT.K ) THEN
*
*                 W := W + C2 * V2
*
                  CALL SGEMM_OMP_TASK( 'No transpose', 'No transpose', 
     $                 M, K, N-K,
...
*              W := W * T  or  W * T**T
*
               CALL STRMM_OMP_TASK( 'Right', 'Upper', TRANS,
     $              'Non-unit', M, K,
     $              ONE, T, LDT, WORK, LDWORK, DEPTH )
*
*              C := C - W * V**T
...
*              W := W * V2
*
               CALL STRMM_OMP_TASK( 'Right', 'Upper', 'No transpose',
     $              'Unit', N,
...
*              W := W * T**T  or  W * T
*
               CALL STRMM_OMP_TASK( 'Right', 'Lower', TRANST,
     $              'Non-unit', N, K,
     $              ONE, T, LDT, WORK, LDWORK, DEPTH )
*
*              C := C - V * W**T
...
*              W := W * V2**T
*     
               CALL STRMM_OMP_TASK( 'Right', 'Upper', 'Transpose',
     $              'Unit', N, K,
...
*              W := W * V2
*
               CALL STRMM_OMP_TASK( 'Right', 'Upper',
     $              'No transpose', 'Unit', M,
...
*              W := W * T  or  W * T**T
*
               CALL STRMM_OMP_TASK( 'Right', 'Lower', TRANS,
     $              'Non-unit', M, K,
     $              ONE, T, LDT, WORK, LDWORK, DEPTH )
*
*              C := C - W * V**T
...
*                 W := W + C2**T * V2**T
*
                  CALL SGEMM_OMP_TASK( 'Transpose', 'Transpose', 
     $                 N, K, M-K, ONE,
...
*                 W := W + C2 * V2**T
*
                  CALL SGEMM_OMP_TASK( 'No transpose', 
     $                 'Transpose', M, K, N-K,
