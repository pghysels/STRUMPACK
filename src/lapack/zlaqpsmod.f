      SUBROUTINE ZLAQPSMOD( M, N, OFFSET, NB, KB, A, LDA, JPVT, TAU,
...
*     .. External Subroutines ..
      EXTERNAL           ZGEMM_OMP_TASK, ZGEMV_OMP_TASK, ZLARFG, ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( K.GT.1 ) THEN
            DO 20 J = 1, K - 1
               F( K, J ) = DCONJG( F( K, J ) )
   20       CONTINUE
            CALL ZGEMV_OMP_TASK( 'No transpose', M-RK+1, K-1,
     $           -CONE, A( RK, 1 ),
     $           LDA, F( K, 1 ), LDF, CONE,
     $           A( RK, K ), 1, DEPTH )
            DO 30 J = 1, K - 1
...
*
         IF( K.LT.N ) THEN
            CALL ZGEMV_OMP_TASK( 'Conjugate transpose', M-RK+1,
     $           N-K, TAU( K ),
...
*
     ...
     $           AUXV( 1 ), 1, DEPTH )
*
            CALL ZGEMV_OMP_TASK( 'No transpose', N, K-1, CONE, 
     $           F( 1, 1 ), LDF,
...
*
         IF( K.LT.N ) THEN
            CALL ZGEMM_OMP_TASK( 'No transpose', 'Conjugate transpose',
