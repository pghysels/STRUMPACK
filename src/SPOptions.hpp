...
namespace strumpack {
  ...
  template<typename real_t> inline real_t default_rel_tol() { return real_t(1.e-6); }
  template<typename real_t> inline real_t default_abs_tol() { return real_t(1.e-10); }
  ...
  public:
    ...
    SPOptions(int argc, char* argv[]) : _argc(argc), _argv(argv) { _hss_opts.set_verbose(false); }
    ...
    void set_nd_param(int nd_param) { assert(nd_param>=0); _nd_param = nd_param; }
    ...
    void set_mc64job(int job) { if (job < 0 || job > 5) std::cerr << "# WARNING: invalid mc64 job number" << std::endl; _mc64job = job; }
    ...
    void set_HSS_min_front_size(int s) { assert(_hss_min_front_size >= 0); _hss_min_front_size = s; }
    void set_HSS_min_sep_size(int s) { assert(_hss_min_sep_size >= 0); _hss_min_sep_size = s; }
    void set_separator_ordering_level(int l) { assert(l >= 0); _sep_order_level = l; }
	...
	{NULL, 0, NULL, 0}
      ...
      opterr = 0;
      while ((c = getopt_long_only(argc, argv, "hvq", long_options, &option_index)) != -1) {
	switch (c) {
	case 1: { std::istringstream iss(optarg); iss >> _maxit; set_maxit(_maxit); } break;
	case 2: { std::istringstream iss(optarg); iss >> _rel_tol; set_rel_tol(_rel_tol); } break;
	case 3: { std::istringstream iss(optarg); iss >> _abs_tol; set_abs_tol(_abs_tol); } break;
	case 4: { std::string s; std::istringstream iss(optarg); iss >> s;
	    if (s.compare("auto") == 0) set_Krylov_solver(KrylovSolver::AUTO);
	    else if (s.compare("direct") == 0) set_Krylov_solver(KrylovSolver::DIRECT);
	    else if (s.compare("refinement") == 0) set_Krylov_solver(KrylovSolver::REFINE);
	    else if (s.compare("pgmres") == 0) set_Krylov_solver(KrylovSolver::PREC_GMRES);
	    else if (s.compare("gmres") == 0) set_Krylov_solver(KrylovSolver::GMRES);
	    else if (s.compare("pbicgstab") == 0) set_Krylov_solver(KrylovSolver::PREC_BICGSTAB);
	    else if (s.compare("bicgstab") == 0) set_Krylov_solver(KrylovSolver::BICGSTAB);
	    else std::cerr << "# WARNING: Krylov solver not recognized, using default" << std::endl;
	} break;
	case 5: { std::istringstream iss(optarg); iss >> _gmres_restart; set_gmres_restart(_gmres_restart); } break;
	case 6: { std::string s; std::istringstream iss(optarg); iss >> s;
	    if (s.compare("modified") == 0) set_GramSchmidt_type(GramSchmidtType::MODIFIED);
	    else if (s.compare("classical") == 0) set_GramSchmidt_type(GramSchmidtType::CLASSICAL);
	    else std::cerr << "# WARNING: Gram-Schmidt type not recognized, use 'modified' or classical" << std::endl;
	} break;
	case 7: { std::string s; std::istringstream iss(optarg); iss >> s;
	    if (s.compare("metis") == 0) set_reordering_method(ReorderingStrategy::METIS);
	    else if (s.compare("parmetis") == 0) set_reordering_method(ReorderingStrategy::PARMETIS);
	    else if (s.compare("scotch") == 0) set_reordering_method(ReorderingStrategy::SCOTCH);
	    else if (s.compare("ptscotch") == 0) set_reordering_method(ReorderingStrategy::PTSCOTCH);
	    else if (s.compare("geometric") == 0) set_reordering_method(ReorderingStrategy::GEOMETRIC);
	    else if (s.compare("rcm") == 0) set_reordering_method(ReorderingStrategy::RCM);
	    else std::cerr << "# WARNING: matrix reordering strategy not recognized, use 'metis', 'parmetis', 'scotch', 'ptscotch', 'geometric' or 'rcm'" << std::endl;
	} break;
	case 8: { std::istringstream iss(optarg); iss >> _nd_param; set_nd_param(_nd_param); } break;
	...
	case 17: { std::istringstream iss(optarg); iss >> _mc64job; set_mc64job(_mc64job); } break;
	...
	case 22: { std::istringstream iss(optarg); iss >> _hss_min_front_size; set_HSS_min_front_size(_hss_min_front_size); } break;
	case 23: { std::istringstream iss(optarg); iss >> _hss_min_sep_size; set_HSS_min_sep_size(_hss_min_sep_size); } break;
	case 24: { std::istringstream iss(optarg); iss >> _sep_order_level; set_separator_ordering_level(_sep_order_level); } break;
	...
	default: break;
      ...
      // 	std::cerr << "# WARNING STRUMPACK: There were unrecognized options." << std::endl;
    ...
    void describe_options() const {
      ...
      std::cout	<< "#   --sp_rel_tol real_t (default " << rel_tol() << ")" << std::endl;
      std::cout	<< "#          Krylov relative (preconditioned) residual stopping tolerance" << std::endl;
      std::cout	<< "#   --sp_abs_tol real_t (default " << abs_tol() << ")" << std::endl;
      std::cout	<< "#          Krylov absolute (preconditioned) residual stopping tolerance" << std::endl;
      std::cout	<< "#   --sp_Krylov_solver auto|direct|refinement|pgmres|gmres|pbicgstab|bicgstab" << std::endl;
      std::cout	<< "#          default: auto (refinement when no HSS, pgmres (preconditioned) with HSS compression)" << std::endl;
      std::cout	<< "#   --sp_gmres_restart int (default " << gmres_restart() << ")" << std::endl;
      std::cout	<< "#          gmres restart length" << std::endl;
      std::cout	<< "#   --sp_GramSchmidt_type [modified|classical]" << std::endl;
      std::cout	<< "#          Gram-Schmidt type for GMRES" << std::endl;
      std::cout	<< "#   --sp_reordering_method metis|scotch|parmetis|ptscotch|rcm|geometric" << std::endl;
      std::cout	<< "#          Code for nested dissection." << std::endl;
      std::cout	<< "#          Geometric only works on regular meshes and you need to provide the sizes." << std::endl;
      std::cout	<< "#   --sp_nd_param int (default " << _nd_param << ")" << std::endl;
      std::cout	<< "#   --sp_enable_METIS_NodeNDP (default " << std::boolalpha << use_METIS_NodeNDP() << ")" << std::endl;
      std::cout	<< "#          use undocumented Metis routine NodeNDP instead of NodeND" << std::endl;
      std::cout	<< "#   --sp_disable_METIS_NodeNDP (default " << std::boolalpha << (!use_METIS_NodeNDP()) << ")" << std::endl;
      std::cout	<< "#          use Metis routine NodeND instead of the undocumented NodeNDP" << std::endl;
      std::cout	<< "#   --sp_enable_METIS_NodeND (default " << std::boolalpha << use_METIS_NodeND() << ")" << std::endl;
      std::cout	<< "#          use Metis routine NodeND instead of the undocumented NodeNDP" << std::endl;
      std::cout	<< "#   --sp_disable_METIS_NodeND (default " << std::boolalpha << !use_METIS_NodeND() << ")" << std::endl;
      std::cout	<< "#          use undocumented Metis routine NodeNDP instead of NodeND" << std::endl;
      std::cout	<< "#   --sp_enable_MUMPS_SYMQAMD (default " << std::boolalpha << use_MUMPS_SYMQAMD() << ")" << std::endl;
      std::cout	<< "#   --sp_disable_MUMPS_SYMQAMD (default " << std::boolalpha << !use_MUMPS_SYMQAMD() << ")" << std::endl;
      std::cout	<< "#   --sp_enable_agg_amalg (default " << std::boolalpha << use_agg_amalg() << ")" << std::endl;
      std::cout	<< "#   --sp_disable_agg_amalg (default " << std::boolalpha << !use_agg_amalg() << ")" << std::endl;
      std::cout << "#   --sp_mc64job int [0-5] (default " << mc64job() << ")" << std::endl;
      std::cout	<< "#   --sp_enable_hss (default " << std::boolalpha << use_HSS() << ")" << std::endl;
      std::cout	<< "#   --sp_disable_hss (default " << std::boolalpha << !use_HSS() << ")" << std::endl;
      std::cout << "#   --sp_hss_min_front_size int (default " << HSS_min_front_size() << ")" << std::endl;
      std::cout << "#          minimum size of front for HSS compression" << std::endl;
      std::cout << "#   --sp_hss_min_sep_size int (default " << HSS_min_sep_size() << ")" << std::endl;
      std::cout << "#          minimum size of the separator for HSS compression of the front" << std::endl;
      std::cout << "#   --sp_separator_ordering_level (default " << separator_ordering_level() << ")" << std::endl;
      std::cout << "#   --sp_enable_indirect_sampling" << std::endl;
      std::cout << "#   --sp_disable_indirect_sampling" << std::endl;
      std::cout << "#   --sp_verbose or -v (default " << verbose() << ")" << std::endl;
      std::cout << "#   --sp_quiet or -q (default " << !verbose() << ")" << std::endl;
