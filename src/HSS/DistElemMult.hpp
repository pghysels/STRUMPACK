  ...
  namespace HSS {
    ...
    public:
      ...
      DistElemMult(const DistM_t& A, int ctxt_all, MPI_Comm comm)
        : _A(A), _Ag(A.all_gather(ctxt_all)), _ctxt(A.ctxt()),
          _ctxt_all(ctxt_all), _comm(comm) { }
      const DistM_t& _A;
      const DenseM_t _Ag;
      ...
      void operator()(const std::vector<size_t>& I,
                      const std::vector<size_t>& J, DistM_t& B) {
        if (!B.active()) return;
    ...
    public:
      LocalElemMult(const delem_t& Aelem,
                    std::pair<std::size_t,std::size_t>& offset, int ctxt)
