...
namespace strumpack {
  namespace HSS {
    ...
    template<typename scalar_t> class HSSMatrixMPI : public HSSMatrixBase<scalar_t> {
    ...
    public:
      ...
      HSSMatrixMPI(const HSSPartitionTree& t, const DistM_t& A, const opts_t& opts, MPI_Comm c);
      HSSMatrixMPI(std::size_t m, std::size_t n, const dmult_t& Amult, int Actxt, const delem_t& Aelem,
		   const opts_t& opts, MPI_Comm c);
      HSSMatrixMPI(const HSSPartitionTree& t, const dmult_t& Amult, int Actxt, const delem_t& Aelem,
		   const opts_t& opts, MPI_Comm c);
      ~HSSMatrixMPI();

      const HSSMatrixBase<scalar_t>* child(int c) const { return this->_ch[c].get(); }
      HSSMatrixBase<scalar_t>* child(int c) { return this->_ch[c].get(); }

      int ctxt() const { return _ctxt; }
      int ctxt(int) const { return _ctxt; }  // this overwrites the one in te Base class
      ...
      void compress(const dmult_t& Amult, const delem_t& Aelem, const opts_t& opts, int Actxt=-1);
      ...
      void forward_solve(const HSSFactorsMPI<scalar_t>& ULV, WorkSolveMPI<scalar_t>& w,
			 const DistM_t& b, bool partial) const;
      void backward_solve(const HSSFactorsMPI<scalar_t>& ULV, WorkSolveMPI<scalar_t>& w, DistM_t& x) const;

      DistM_t apply(const DistM_t& b) const;
      DistM_t applyC(const DistM_t& b) const;

      scalar_t get(std::size_t i, std::size_t j) const;
      DistM_t extract(const std::vector<std::size_t>& I, const std::vector<std::size_t>& J,
		      int Bctxt, int Bprows, int Bpcols) const;
      void extract_add(const std::vector<std::size_t>& I, const std::vector<std::size_t>& J,
      ...
      void print_info(std::ostream &out=std::cout, std::size_t roff=0, std::size_t coff=0) const;

      DistM_t dense(int ctxt) const;

      const TreeLocalRanges& tree_ranges() const { return _ranges; }
      void to_block_row(const DistM_t& A, DenseM_t& sub_A, DistM_t& leaf_A) const;
      void allocate_block_row(int d, DenseM_t& sub_A, DistM_t& leaf_A) const;
      void from_block_row(DistM_t& A, const DenseM_t& sub_A, const DistM_t& leaf_A, int lctxt) const;
    ...
    private:
      ...
      HSSMatrixMPI(std::size_t m, std::size_t n, const opts_t& opts, MPI_Comm c, int P, bool dup_comm,
		   std::size_t roff, std::size_t coff);
      HSSMatrixMPI(const HSSPartitionTree& t, const opts_t& opts, MPI_Comm c, int P, bool dup_comm,
		   std::size_t roff, std::size_t coff);
      void setup_hierarchy(const opts_t& opts, MPI_Comm c, int P, bool dup_comm,
			   std::size_t roff, std::size_t coff);
      void setup_hierarchy(const HSSPartitionTree& t, const opts_t& opts, MPI_Comm c, int P, bool dup_comm,
			   std::size_t roff, std::size_t coff);
      void setup_contexts(int P);
      void setup_ranges(std::size_t roff, std::size_t coff);

      void compress_original_nosync(const dmult_t& Amult, const delem_t& Aelem, const opts_t& opts, int Actxt=-1);
      void compress_original_sync(const dmult_t& Amult, const delem_t& Aelem, const opts_t& opts, int Actxt=-1);
      void compress_stable_nosync(const dmult_t& Amult, const delem_t& Aelem, const opts_t& opts, int Actxt=-1);
      void compress_stable_sync(const dmult_t& Amult, const delem_t& Aelem, const opts_t& opts, int Actxt=-1);

      void compress_recursive_original(DistSamples<scalar_t>& RS, const delem_t& Aelem, const opts_t& opts,
				       WorkCompressMPI<scalar_t>& w, int dd);
      void compress_recursive_stable(DistSamples<scalar_t>& RS, const delem_t& Aelem, const opts_t& opts,
				     WorkCompressMPI<scalar_t>& w, int d, int dd);
      void compute_local_samples(const DistSamples<scalar_t>& RS, WorkCompressMPI<scalar_t>& w, int dd);
      bool compute_U_V_bases(int d, const opts_t& opts, WorkCompressMPI<scalar_t>& w);
      ...
      void extract_level(const delem_t& Aelem, const opts_t& opts, WorkCompressMPI<scalar_t>& w, int lvl);
      void get_extraction_indices(std::vector<std::vector<std::size_t>>& I, std::vector<std::vector<std::size_t>>& J,
				  WorkCompressMPI<scalar_t>& w, int& self, int lvl);
      void allgather_extraction_indices(std::vector<std::vector<std::size_t>>& lI, std::vector<std::vector<std::size_t>>& lJ,
					std::vector<std::vector<std::size_t>>& I, std::vector<std::vector<std::size_t>>& J,
					int& before, int self, int& after);
      void extract_D_B(const delem_t& Aelem, int lctxt, const opts_t& opts, WorkCompressMPI<scalar_t>& w, int lvl);

      void factor_recursive(HSSFactorsMPI<scalar_t>& f, WorkFactorMPI<scalar_t>& w, int local_ctxt,
			    bool isroot, bool partial) const;

      void solve_fwd(const HSSFactorsMPI<scalar_t>& ULV, const DistSubLeaf<scalar_t>& b,
		     WorkSolveMPI<scalar_t>& w, bool partial, bool isroot) const;
      void solve_bwd(const HSSFactorsMPI<scalar_t>& ULV, DistSubLeaf<scalar_t>& x,
		     WorkSolveMPI<scalar_t>& w, bool isroot) const;

      void apply_fwd(const DistSubLeaf<scalar_t>& B, WorkApplyMPI<scalar_t>& w, bool isroot) const;
      void apply_bwd(const DistSubLeaf<scalar_t>& B, scalar_t beta, DistSubLeaf<scalar_t>& C,
		     WorkApplyMPI<scalar_t>& w, bool isroot) const;
      void applyT_fwd(const DistSubLeaf<scalar_t>& B, WorkApplyMPI<scalar_t>& w, bool isroot) const;
      void applyT_bwd(const DistSubLeaf<scalar_t>& B, scalar_t beta, DistSubLeaf<scalar_t>& C,
		      WorkApplyMPI<scalar_t>& w, bool isroot) const;

      void extract_fwd(WorkExtractMPI<scalar_t>& w, int lctxt, bool odiag) const;
      ...
      void apply_UV_big(DistSubLeaf<scalar_t>& Theta, DistM_t& Uop, DistSubLeaf<scalar_t>& Phi, DistM_t& Vop) const;
      ...
      int Pl(int P) const { return Pl(this->rows(), this->_ch[0]->rows(), this->_ch[1]->rows(), P); }
      int Pr(int P) const { return Pr(this->rows(), this->_ch[0]->rows(), this->_ch[1]->rows(), P); }

      template<typename T> friend
      void apply_HSS(Trans ta, const HSSMatrixMPI<T>& a, const DistributedMatrix<T>& b,
		     T beta, DistributedMatrix<T>& c);
    ...
    template<typename scalar_t> HSSMatrixMPI<scalar_t>::HSSMatrixMPI
    (const HSSPartitionTree& t, const DistM_t& A, const opts_t& opts, MPI_Comm c)
      : HSSMatrixBase<scalar_t>(A.rows(), A.cols(), true) {
    ...
    template<typename scalar_t> HSSMatrixMPI<scalar_t>::HSSMatrixMPI
    (std::size_t m, std::size_t n, const dmult_t& Amult, int Actxt, const delem_t& Aelem,
     const opts_t& opts, MPI_Comm c) : HSSMatrixBase<scalar_t>(m, n, true) {
      setup_hierarchy(opts, c, mpi_nprocs(c), true, 0, 0);
      setup_contexts(mpi_nprocs(c));
      setup_ranges(0, 0);
    ...
    template<typename scalar_t> HSSMatrixMPI<scalar_t>::HSSMatrixMPI
    (const HSSPartitionTree& t, const dmult_t& Amult, int Actxt, const delem_t& Aelem,
     const opts_t& opts, MPI_Comm c) : HSSMatrixBase<scalar_t>(t.size, t.size, true) {
      setup_hierarchy(t, opts, c, mpi_nprocs(c), true, 0, 0);
      setup_contexts(mpi_nprocs(c));
      setup_ranges(0, 0);
      compress(Amult, Aelem, opts, Actxt);
    ...
    /** private constructor */
    template<typename scalar_t> HSSMatrixMPI<scalar_t>::HSSMatrixMPI
    (std::size_t m, std::size_t n, const opts_t& opts, MPI_Comm c, int P, bool dup_comm,
     std::size_t roff, std::size_t coff) : HSSMatrixBase<scalar_t>(m, n, true) {
    ...
    /** private constructor */
    template<typename scalar_t> HSSMatrixMPI<scalar_t>::HSSMatrixMPI
    (const HSSPartitionTree& t, const opts_t& opts, MPI_Comm c, int P, bool dup_comm,
     std::size_t roff, std::size_t coff) : HSSMatrixBase<scalar_t>(t.size, t.size, true) {
      setup_hierarchy(t, opts, c, P, dup_comm, roff, coff);
      setup_contexts(P);
      setup_ranges(roff, coff);
    }

    template<typename scalar_t> void HSSMatrixMPI<scalar_t>::setup_contexts(int P) {
      _nprocs = P; // P is not always equal to mpi_nprocs(_comm), _comm can be MPI_COMM_NULL
    ...
    (const opts_t& opts, MPI_Comm c, int P, bool dup_comm, std::size_t roff, std::size_t coff) {
      auto m = this->rows();
      auto n = this->cols();
      if (c == MPI_COMM_NULL || !dup_comm) _comm = c;
      else MPI_Comm_dup(c, &_comm);
      if (m > std::size_t(opts.leaf_size()) || n > std::size_t(opts.leaf_size())) {
	this->_ch.reserve(2);
	auto pl = Pl(m, m/2, m-m/2, P);
	auto pr = Pr(m, m/2, m-m/2, P);
	if (pl > 1) {
	  auto c0 = mpi_sub_comm(_comm, 0, pl);
	  this->_ch.emplace_back(new HSSMatrixMPI<scalar_t>(m/2, n/2, opts, c0, pl, false, roff, coff));
	} else {
	  bool act = (_comm != MPI_COMM_NULL) && (mpi_rank(_comm) == 0);
	  this->_ch.emplace_back(new HSSMatrix<scalar_t>(m/2, n/2, opts, act));
	}
	if (pr > 1) {
	  auto c1 = mpi_sub_comm(_comm, pl, pr);
	  this->_ch.emplace_back(new HSSMatrixMPI<scalar_t>(m-m/2, n-n/2, opts, c1, pr, false, roff+m/2, coff+n/2));
	} else {
	  bool act = (_comm != MPI_COMM_NULL) && (mpi_rank(_comm) == pl);
	  this->_ch.emplace_back(new HSSMatrix<scalar_t>(m-m/2, n-n/2, opts, act));
	}
      }
    }
	...
	if (pl > 1) {
	  auto c0 = mpi_sub_comm(_comm, 0, pl);
	  this->_ch.emplace_back(new HSSMatrixMPI<scalar_t>(t.c[0], opts, c0, pl, false, roff, coff));
	} else {
	  bool act = (_comm != MPI_COMM_NULL) && (mpi_rank(_comm) == 0);
	...
	if (pr > 1) {
	  ...
	  this->_ch.emplace_back
	    (new HSSMatrixMPI<scalar_t>(t.c[1], opts, c1, pr, false, roff+t.c[0].size, coff+t.c[0].size));
	} else {
	  bool act = (_comm != MPI_COMM_NULL) && (mpi_rank(_comm) == pl);
    ...
    template<typename scalar_t> std::size_t HSSMatrixMPI<scalar_t>::rank() const {
    ...
    template<typename scalar_t> std::size_t HSSMatrixMPI<scalar_t>::total_memory() const {
      std::size_t mtot, m=memory();
      MPI_Allreduce(&m, &mtot, 1, mpi_type<std::size_t>(), MPI_SUM, _comm);
      return mtot;
    }
    template<typename scalar_t> std::size_t HSSMatrixMPI<scalar_t>::memory() const {
      if (!this->active()) return 0;
    ...
    template<typename scalar_t> std::size_t HSSMatrixMPI<scalar_t>::total_nonzeros() const {
      std::size_t nnztot, nnz=nonzeros();
      MPI_Allreduce(&nnz, &nnztot, 1, mpi_type<std::size_t>(), MPI_SUM, _comm);
      return nnztot;
    }
    template<typename scalar_t> std::size_t HSSMatrixMPI<scalar_t>::nonzeros() const {
      if (!this->active()) return 0;
    ...
    template<typename scalar_t> std::size_t HSSMatrixMPI<scalar_t>::max_levels() const {
      std::size_t ltot, l=levels();
      MPI_Allreduce(&l, &ltot, 1, mpi_type<std::size_t>(), MPI_MAX, _comm);
      return ltot;
    }
    template<typename scalar_t> std::size_t HSSMatrixMPI<scalar_t>::levels() const {
      if (!this->active()) return 0;
      std::size_t lvls = 0;
	...
	is included in comm? */
    template<typename scalar_t> DistributedMatrix<scalar_t> HSSMatrixMPI<scalar_t>::dense(int ctxt) const {
    ...
    template<typename scalar_t> void HSSMatrixMPI<scalar_t>::communicate_child_data
    ...
    (std::ostream &out, std::size_t roff, std::size_t coff) const {
      if (!this->active()) return;
      if (!mpi_rank(_comm)) {
	out << "rank = " << mpi_rank(_comm) << "/" << mpi_rank(MPI_COMM_WORLD) << " P=" << mpi_nprocs(_comm)
