...
namespace strumpack {
  namespace HSS {
    ...
    template<typename scalar_t> class WorkCompressMPI : public WorkCompressBase<scalar_t> {
    public:
	...
	if (!w_seq) w_seq = std::unique_ptr<WorkCompress<scalar_t>>(new WorkCompress<scalar_t>());
	w_seq->lvl = this->lvl;
      }
    };
	...
	for (std::size_t p=0; p<_ranges.size()/5; p++)
	  std::cout << rlo(p) << "," << rhi(p) << "/" << clo(p) << "," << chi(p) << "/" << leaf_procs(p) << " ";
	std::cout << "];" << std::endl;
      }
    ...
    public:
      ...
      DistributedMatrix<scalar_t> Dt;  // (U.cols x U.cols) \tilde(D)
      DistributedMatrix<scalar_t> Vt1; // (U.cols x V.cols) bottom part of \tilde{V}
	...
	std::size_t nnz = _L.nonzeros() + _Vt0.nonzeros() + _W1.nonzeros() + _Q.nonzeros() + _D.nonzeros();
    ...
    private:
      ...
      DistributedMatrix<scalar_t> _L;   // (U.rows-U.cols x U.rows-U.cols), empty at the root
      DistributedMatrix<scalar_t> _Vt0; // (U.rows-U.cols x V.cols)
                                        // at the root, _Vt0 stored Vhat
      DistributedMatrix<scalar_t> _W1;  // (U.cols x U.rows) bottom part of W
                                        // if (U.rows == U.cols) then W == I and is not stored!
      DistributedMatrix<scalar_t> _Q;   // (U.rows x U.rows) Q from LQ(W0)
                                        // if (U.rows == U.cols) then Q == I and is not stored!
      DistributedMatrix<scalar_t> _D;   // (U.rows x U.rows) at the root holds LU(D), else empty
    ...
    public:
      ...
      void split_extraction_sets(const std::pair<std::size_t,std::size_t>& dim) {
	if (c.empty()) {
	  c.resize(2);
	...
	MPI_Bcast(c[0].ycols.data(), c0ycols, mpi_type<std::size_t>(), rch0, comm);
	MPI_Bcast(c[1].ycols.data(), c1ycols, mpi_type<std::size_t>(), rch1, comm);
      }
	...
	std::copy(c[1].ycols.begin(), c[1].ycols.end(), ycols.begin()+c0ycols);
      }
    };
