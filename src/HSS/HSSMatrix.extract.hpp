...
namespace strumpack {
  namespace HSS {
    ...
    template<typename scalar_t> scalar_t HSSMatrix<scalar_t>::get(std::size_t i, std::size_t j) const {
    ...
    template<typename scalar_t> DenseMatrix<scalar_t> HSSMatrix<scalar_t>::extract
    ...
    (const std::vector<std::size_t>& I, const std::vector<std::size_t>& J, DenseM_t& B) const {
	...
	if (tasked) {
#pragma omp task default(shared) final(depth >= params::task_recursion_cutoff_level-1) mergeable
	  this->_ch[0]->extract_fwd(w.c[0], odiag || !w.c[1].I.empty(), depth+1);
#pragma omp task default(shared) final(depth >= params::task_recursion_cutoff_level-1) mergeable
	  this->_ch[1]->extract_fwd(w.c[1], odiag || !w.c[0].I.empty(), depth+1);
#pragma omp taskwait
	} else {
	  this->_ch[0]->extract_fwd(w.c[0], odiag || !w.c[1].I.empty(), depth+1);
	  this->_ch[1]->extract_fwd(w.c[1], odiag || !w.c[0].I.empty(), depth+1);
	...
	std::copy(w.c[1].ycols.begin(), w.c[1].ycols.end(), w.ycols.begin()+w.c[0].ycols.size());
	if (_V.cols()) {
...
#pragma omp critical
	...
	if (w.z.cols() && _U.cols()) {
	  ...
	  gemm(Trans::N, Trans::N, scalar_t(1), _U.extract_rows(w.I), w.z, scalar_t(0.), tmp, depth);
...
#pragma omp taskwait
	...
	if (!w.c[1].ycols.empty()) {
	  ...
	  gemm(Trans::N, Trans::N, scalar_t(1.), _B01, w.c[1].y, scalar_t(0.), z00, depth);
	}
	DenseMW_t z01(z0rows, w.z.cols(), w.c[0].z, 0, w.c[1].y.cols());
	if (U.cols()) {
	  DenseMW_t U0(z0rows, U.cols(), U, 0, 0);
	  gemm(Trans::N, Trans::N, scalar_t(1.), U0, w.z, scalar_t(0.), z01, depth);
	} else z01.zero();
	...
	if (!w.c[0].ycols.empty()) {
	  ...
	  gemm(Trans::N, Trans::N, scalar_t(1.), _B10, w.c[0].y, scalar_t(0.), z10, depth);
	}
	DenseMW_t z11(z1rows, w.z.cols(), w.c[1].z, 0, w.c[0].y.cols());
	if (U.cols()) {
	  DenseMW_t U1(z1rows, U.cols(), U, this->_ch[0]->U_rank(), 0);
	  gemm(Trans::N, Trans::N, scalar_t(1.), U1, w.z, scalar_t(0.), z11, depth);
	} else z11.zero();
