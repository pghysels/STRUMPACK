  ...
  namespace HSS {
    ...
    (const std::vector<std::vector<std::size_t>>& I,
     ...
     int Bctxt, int Bprows, int Bpcols) const {
      ...
      for (std::size_t i=0; i<I.size(); i++) {
        B.emplace_back(Bctxt, I[i].size(), J[i].size());
        B[i].zero();
      }
    ...
    (const std::vector<std::vector<std::size_t>>& I,
     ...
     std::vector<DistM_t>& B, int Bprows, int Bpcols) const {
      ...
      w.z.reserve(nb);
      for (std::size_t k=0; k<nb; k++) {
        w.z.emplace_back(_ctxt, this->U_rank(), w.ycols[k].size());
        w.z[k].zero();
      }
    ...
    (WorkExtractBlocksMPI<scalar_t>& w, int lctxt,
     std::vector<bool>& odiag) const {
      ...
      if (this->leaf()) {
        for (std::size_t k=0; k<nb; k++) {
          if (odiag[k])
            w.y[k] = _V.extract_rows(w.J[k], _comm).transpose();
            ...
            else w.c[1].ycols[k].push_back(w.ycols[k][c]);
        {
          ...
          this->_ch[0]->extract_fwd(w.c[0], lctxt, odiag0);
          this->_ch[1]->extract_fwd(w.c[1], lctxt, odiag1);
        }
        ...
        if (this->V_rank()) {
          ...
          for (std::size_t k=0; k<nb; k++) {
            if (!odiag[k]) continue;
            w.y[k] = DistM_t(_ctxt, 0, w.J[k].size());
          }
        }
      }
    }
      ...
      if (this->leaf()) {
        for (std::size_t k=0; k<nb; k++) {
          ...
          if (w.z[k].cols() && _U.cols()) {
            DistM_t tmp(_ctxt, w.I[k].size(), w.z[k].cols());
            {
              auto Uex = _U.extract_rows(w.I[k], _comm);
        ...
        for (std::size_t k=0; k<nb; k++) {
          if (!w.c[0].I[k].empty()) {
            ...
            w.c[0].z[k] = DistM_t(this->_ch[0]->ctxt(lctxt), z0rows, z0cols);
            if (!w.c[1].ycols[k].empty()) {
              DistM_t z00(_ctxt, z0rows, w.c[1].ycols[k].size());
              DistM_t wc1y(_ctxt, _B01.cols(), w.c[1].ycols[k].size());
              copy(_B01.cols(), w.c[1].ycols[k].size(), w.c[1].y[k], 0, 0,
                   wc1y, 0, 0, _ctxt_all);
              ...
              STRUMPACK_EXTRACTION_FLOPS
                (gemm_flops(Trans::N, Trans::N, scalar_t(1.), _B01, wc1y,
                            scalar_t(0.)));
              copy(z0rows, w.c[1].ycols[k].size(), z00, 0, 0,
                   w.c[0].z[k], 0, 0, _ctxt_all);
            }
            if (this->U_rank()) {
              DistM_t z01(_ctxt, z0rows, w.z[k].cols());
              ...
              copy(z0rows, w.z[k].cols(), z01, 0, 0,
                   w.c[0].z[k], 0, w.c[1].ycols[k].size(), _ctxt_all);
            } else {
          ...
          if (!w.c[1].I[k].empty()) {
            ...
            w.c[1].z[k] = DistM_t(this->_ch[1]->ctxt(lctxt), z1rows, z1cols);
            if (!w.c[0].ycols[k].empty()) {
              DistM_t z10(_ctxt, z1rows, w.c[0].ycols[k].size());
              DistM_t wc0y(_ctxt, _B10.cols(), w.c[0].ycols[k].size());
              copy(_B10.cols(), w.c[0].ycols[k].size(), w.c[0].y[k], 0, 0,
                   wc0y, 0, 0, _ctxt_all);
              ...
              STRUMPACK_EXTRACTION_FLOPS
                (gemm_flops(Trans::N, Trans::N, scalar_t(1.), _B10, wc0y,
                            scalar_t(0.)));
              copy(z1rows, w.c[0].ycols[k].size(), z10, 0, 0,
                   w.c[1].z[k], 0, 0, _ctxt_all);
            }
            if (this->U_rank()) {
              DistM_t z11(_ctxt, z1rows, w.z[k].cols());
              DistMW_t U1(z1rows, this->U_rank(), U, this->_ch[0]->U_rank(), 0);
              gemm(Trans::N, Trans::N, scalar_t(1.),
                   U1, w.z[k], scalar_t(0.), z11);
              STRUMPACK_EXTRACTION_FLOPS
                (gemm_flops(Trans::N, Trans::N, scalar_t(1.),
                            U1, w.z[k], scalar_t(0.)));
              copy(z1rows, w.z[k].cols(), z11, 0, 0,
                   w.c[1].z[k], 0, w.c[0].ycols[k].size(), _ctxt_all);
            } else {
