  ...
  namespace HSS {
    ...
    (std::vector<std::vector<Triplet<scalar_t>>>& triplets,
     std::vector<DistM_t>& B) const {
      ...
      using Quadlet = Quadlet<scalar_t>;
      ...
      std::vector<int> nb_destr(nb*(maxBrows+maxBcols), -1);
      std::vector<int> ssize(P, 0);
      for (std::size_t k=0; k<nb; k++) {
        auto destr = &nb_destr[k*(maxBrows+maxBcols)];
        auto destc = destr + maxBrows;
        for (auto& t : triplets[k]) {
          assert(t.r >= 0 && t.c >= 0);
          assert(t.r < B[k].rows() && t.c < B[k].cols());
          auto dr = destr[t.r];
     ...
     std::vector<bool>& odiag) const {
      ...
      if (this->leaf()) {
            ...
            else w.c[1].ycols[k].push_back(w.ycols[k][c]);
        {
          ...
          this->ch_[0]->extract_fwd(w.c[0], lg, odiag0);
          this->ch_[1]->extract_fwd(w.c[1], lg, odiag1);
        }
        ...
        if (this->V_rank()) {
          for (std::size_t k=0; k<nb; k++) {
            ...
            copy(this->ch_[0]->V_rank(), w.c[0].ycols[k].size(),
                 w.c[0].y[k], 0, 0, y01, 0, 0, grid()->ctxt_all());
            copy(this->ch_[1]->V_rank(), w.c[1].ycols[k].size(),
                 w.c[1].y[k], 0, 0, y01,
                 this->ch_[0]->V_rank(), w.c[0].ycols[k].size(),
                 grid()->ctxt_all());
            w.y[k] = _V.applyC(y01);
      ...
      if (this->leaf()) {
        ...
        for (std::size_t k=0; k<nb; k++) {
          if (!w.c[0].I[k].empty()) {
            ...
            w.c[0].z[k] = DistM_t(this->ch_[0]->grid(lg), z0rows, z0cols);
          ...
          if (!w.c[1].I[k].empty()) {
            ...
            w.c[1].z[k] = DistM_t(this->ch_[1]->grid(lg), z1rows, z1cols);
