        ...
        if (!isroot) {
          w.tmp1 = _V.applyC
            (vconcat(B.cols(), this->_B10.cols(), this->_B01.cols(),
                     w.c[0].tmp1, w.c[1].tmp1, grid(), grid()));
    ...
    (const DistSubLeaf<scalar_t>& B, scalar_t beta, DistSubLeaf<scalar_t>& C,
     WorkApplyMPI<scalar_t>& w, bool isroot, long long int flops) const {
      ...
      if (this->leaf()) {
        ...
        auto n = B.cols();
        DistM_t c0tmp1(grid(), _B10.cols(), n, w.c[0].tmp1, grid());
        DistM_t c1tmp1(grid(), _B01.cols(), n, w.c[1].tmp1, grid());
        ...
        if (isroot || !this->U_rank()) {
          ...
          copy(this->_B01.rows(), n, tmp, 0, 0, c0tmp2, 0, 0, grid());
          copy(this->_B10.rows(), n, tmp, this->_B01.rows(), 0, c1tmp2, 0, 0, grid());
        ...
        w.c[0].tmp2 = DistM_t(w.c[0].tmp1.grid(), _B01.rows(), n, c0tmp2, grid());
        w.c[1].tmp2 = DistM_t(w.c[1].tmp1.grid(), _B10.rows(), n, c1tmp2, grid());
    ...
    (const DistSubLeaf<scalar_t>& B, scalar_t beta, DistSubLeaf<scalar_t>& C,
     WorkApplyMPI<scalar_t>& w, bool isroot, long long int flops) const {
      ...
      if (this->leaf()) {
        ...
        auto n = B.cols();
        DistM_t c0tmp1(grid(), _B01.rows(), n, w.c[0].tmp1, grid());
        DistM_t c1tmp1(grid(), _B10.rows(), n, w.c[1].tmp1, grid());
        ...
        if (isroot || !this->V_rank()) {
          ...
          copy(c0tmp2.rows(), n, tmp, 0, 0, c0tmp2, 0, 0, grid());
          copy(c1tmp2.rows(), n, tmp, c0tmp2.rows(), 0, c1tmp2, 0, 0, grid());
          gemm(Trans::C, Trans::N, scalar_t(1.), _B10, c1tmp1,
               scalar_t(1.), c0tmp2);
          gemm(Trans::C, Trans::N, scalar_t(1.), _B01, c0tmp1,
               scalar_t(1.), c1tmp2);
        ...
        w.c[0].tmp2 = DistM_t(w.c[0].tmp1.grid(), c0tmp2.rows(), n, c0tmp2, grid());
        w.c[1].tmp2 = DistM_t(w.c[1].tmp1.grid(), c1tmp2.rows(), n, c1tmp2, grid());
