...
namespace strumpack {
  namespace HSS {
    ...
    template<typename scalar_t> HSSFactorsMPI<scalar_t> HSSMatrixMPI<scalar_t>::factor() const {
    ...
    template<typename scalar_t> HSSFactorsMPI<scalar_t> HSSMatrixMPI<scalar_t>::partial_factor() const {
      HSSFactorsMPI<scalar_t> f;
      WorkFactorMPI<scalar_t> w;
    ...
    (HSSFactorsMPI<scalar_t>& f, WorkFactorMPI<scalar_t>& w, int local_ctxt, bool isroot, bool partial) const {
      if (!this->active()) return;
      DistM_t Vh;
      if (!this->leaf()) {
	f._ch.resize(2);
	w.c.resize(2);
	this->_ch[0]->factor_recursive(f._ch[0], w.c[0], local_ctxt, false, partial);
	this->_ch[1]->factor_recursive(f._ch[1], w.c[1], local_ctxt, false, partial);
	auto c0u = _B01.rows();
	auto c1u = _B10.rows();
	auto c0v = _B10.cols();
	auto c1v = _B01.cols();
	...
	if (this->U_rows() > this->U_rank()) {
	  ...
	  // set W1 <- (P^t D)_0
	  copy(this->U_rank(), this->U_rows(), f._D, 0, 0, f._W1, 0, 0, _ctxt_all);
	  DistM_t W0(_ctxt, this->U_rows()-this->U_rank(), this->U_rows());
	  // set W0 <- (P^t D)_1   (bottom part of P^t D)
	  copy(W0.rows(), W0.cols(), f._D, this->U_rank(), 0, W0, 0, 0, _ctxt_all);
	  f._D.clear();
	  ...
	  f._Vt0 = DistM_t(_ctxt, this->U_rows()-this->U_rank(), this->V_rank());
	  w.Vt1 = DistM_t(_ctxt, this->U_rank(), this->V_rank());
	  DistMW_t Q0(this->U_rows()-this->U_rank(), this->U_rows(), f._Q, 0, 0);
	  DistMW_t Q1(this->U_rank(), this->U_rows(), f._Q, Q0.rows(), 0);
	  gemm(Trans::N, Trans::N, scalar_t(1.), Q0, Vh, scalar_t(0.), f._Vt0); // Q0 * Vh
	  gemm(Trans::N, Trans::N, scalar_t(1.), Q1, Vh, scalar_t(0.), w.Vt1);  // Q1 * Vh

	  w.Dt = DistM_t(_ctxt, this->U_rank(), this->U_rank());
	  gemm(Trans::N, Trans::C, scalar_t(1.), f._W1, Q1, scalar_t(0.), w.Dt); // W1 * Q1^c
	} else {
	  w.Vt1 = std::move(Vh);
	  w.Dt = std::move(f._D);
	}
