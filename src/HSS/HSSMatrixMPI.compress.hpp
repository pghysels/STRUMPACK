...
namespace strumpack {
  namespace HSS {
    ...
    template<typename scalar_t> void HSSMatrixMPI<scalar_t>::compress(const DistM_t& A, const opts_t& opts) {
    ...
    template<typename scalar_t> void HSSMatrixMPI<scalar_t>::compress_original_nosync
    (const dmult_t& Amult, const delem_t& Aelem, const opts_t& opts, int Actxt) {
	...
	RS.add_columns(d, opts);
      	if (opts.verbose() && !mpi_rank(_comm))
      	  std::cout << "# compressing with d = " << d << ", d_old = " << d_old << ", tol = " << opts.rel_tol() << std::endl;
      	compress_recursive_original(RS, Aelem, opts, w, d-d_old);
      }
    }

    template<typename scalar_t> void HSSMatrixMPI<scalar_t>::compress_original_sync
    (const dmult_t& Amult, const delem_t& Aelem, const opts_t& opts, int Actxt) {
      WorkCompressMPI<scalar_t> w;
      int d_old = 0, d = opts.d0();
      DistSamples<scalar_t> RS(d, (Actxt!=-1) ? Actxt : _ctxt, *this, Amult, opts);
    ...
    (const delem_t& Aelem, const opts_t& opts, WorkCompressMPI<scalar_t>& w, int lvl) {
	...
	if (w.lvl == lvl && this->is_untouched()) {
	  ...
	  if (mpi_rank(_comm)==0) {
	    ...
	    for (std::size_t i=0; i<this->rows(); i++) I.back().push_back(i+w.offset.first);
	    for (std::size_t j=0; j<this->cols(); j++) J.back().push_back(j+w.offset.second);
	  }
	}
      } else {
    ...
    template<typename scalar_t> void HSSMatrixMPI<scalar_t>::allgather_extraction_indices
    (std::vector<std::vector<std::size_t>>& lI, std::vector<std::vector<std::size_t>>& lJ,
     std::vector<std::vector<std::size_t>>& I, std::vector<std::vector<std::size_t>>& J,
     int& before, int self, int& after) {
      ...
      MPI_Allgather(MPI_IN_PLACE, 0, mpi_type<int>(), rsize, 1, mpi_type<int>(), _comm);
	...
	*ptr++ = lJ[i].size();
	...
	ptr += lJ[i].size();
      ...
      MPI_Allgatherv(MPI_IN_PLACE, 0, mpi_type<std::size_t>(), sbuf, rsize, displs, mpi_type<std::size_t>(), _comm);
      int total = 0;
    ...
    template<typename scalar_t> void HSSMatrixMPI<scalar_t>::compress_recursive_original
	...
	w.split(this->_ch[0]->dims());
	this->_ch[0]->compress_recursive_original(RS, Aelem, opts, w.c[0], dd);
	this->_ch[1]->compress_recursive_original(RS, Aelem, opts, w.c[1], dd);
	communicate_child_data(w);
	if (!this->_ch[0]->is_compressed() || !this->_ch[1]->is_compressed()) return;
	if (this->is_untouched()) {
	  _B01 = DistM_t(_ctxt, w.c[0].Ir.size(), w.c[1].Ic.size());
    ...
    template<typename scalar_t> void HSSMatrixMPI<scalar_t>::compress_level_original
    (DistSamples<scalar_t>& RS, const opts_t& opts, WorkCompressMPI<scalar_t>& w, int dd, int lvl) {
      if (!this->active()) return;
      if (this->leaf()) {
	if (w.lvl < lvl) return;
      } else {
    ...
    template<typename scalar_t> void HSSMatrixMPI<scalar_t>::compute_local_samples
	...
	if (!c_old) {
	  ...
	  copy(this->rows(), dd, RS.leaf_Sr, 0, d_old, wSr_new, 0, 0, _ctxt_all);
	  copy(this->rows(), dd, RS.leaf_Sc, 0, d_old, wSc_new, 0, 0, _ctxt_all);
	  gemm(Trans::N, Trans::N, scalar_t(-1), _D, *wR, scalar_t(1.), wSr_new);
	  gemm(Trans::C, Trans::N, scalar_t(-1), _D, *wR, scalar_t(1.), wSc_new);
	}
      } else {
