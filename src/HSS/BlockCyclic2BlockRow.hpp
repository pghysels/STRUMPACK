      ...
      (const TreeLocalRanges& ranges, const DistributedMatrix<scalar_t>& dist,
       ...
       int lctxt, MPI_Comm comm) {
        ...
        for (int p=0; p<P; p++) {
          ...
          auto pdist = ConstDistributedMatrixWrapperPtr
            (m, d, dist, rbegin, 0);
          int rlo, rhi, clo, chi;
            ...
            if (p == rank) {
              ...
              if (dist.active()) {
                for (int c=clo; c<chi; c++) {
                  ...
                  for (int r=rlo; r<rhi; r++)
                    sbuf[p].emplace_back
                      (dist.rowl2g_fixed(r) - rbegin, gc, dist(r,c));
                }
              }
            }
          } else {
            ...
            if (dist.active()) {
              ...
              for (int c=clo; c<chi; c++) {
                ...
                for (int r=rlo; r<rhi; r++) {
                  ...
                  std::size_t dest = p + ((gr / MB) % leaf_prows) +
                    ((gc / MB) % leaf_pcols) * leaf_prows;
        ...
        if (leaf_procs == 1) { // sub
          for (std::size_t c=0; c<sub.cols(); c++)
            for (std::size_t r=0; r<sub.rows(); r++) {
              auto gr = r + rbegin;
              std::size_t dest = ((gr / MB) % dist_prows) +
                ((c / MB) % dist_pcols) * dist_prows;
