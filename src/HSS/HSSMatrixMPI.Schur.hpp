...
namespace strumpack {
  namespace HSS {
    ...
    (const HSSFactorsMPI<scalar_t>& f, DistM_t& Theta, DistM_t& Vhat, DistM_t& DUB01, DistM_t& Phi) const {
      if (this->leaf()) return;
      auto ch0 = child(0);
      auto ch1 = child(1);
      DistM_t DU(ctxt(), ch0->U_rows(), ch0->U_rank());
      if (auto ch0mpi = dynamic_cast<const HSSMatrixMPI<scalar_t>*>(child(0))) {
	DistM_t chDU;
	...
	DenseM_t chDU;
	if (ch0seq->active())
	  chDU = f._D.gather().solve(ch0seq->_U.dense(), f._piv, ch0seq->_openmp_task_depth);
	copy(ch0->U_rows(), ch0->U_rank(), chDU, 0/*rank ch0*/, DU, 0, 0, _ctxt_all);
      }
	...
	Phi_br(_phi.cols(), ch1, ctxt_loc());
      ...
      copy(Theta.rows(), Theta.cols(), Theta_ch, 0, 0, Theta, 0, 0, _ctxt_all);
    ...
    (const DistM_t& Theta, const DistM_t& Vhat, const DistM_t& DUB01, const DistM_t& Phi,
     const DistM_t& ThetaVhatC, const DistM_t& VhatCPhiC, const DistM_t& R, DistM_t& Sr, DistM_t& Sc) const {
      ...
      DistSubLeaf<scalar_t> R_br(n, ch1, ctxt_loc(), RS_ch1), Sr_br(n, ch1, ctxt_loc()),
	Sc_br(n, ch1, ctxt_loc());
	...
	gemm(Trans::C, Trans::N, scalar_t(1.), Vhat, DUB01, scalar_t(0.), VtDUB01);
	DistM_t tmpr(ctxt(), child(0)->V_rank(), n);
	gemm(Trans::N, Trans::N, scalar_t(1.), VtDUB01, wrtmp, scalar_t(0.), tmpr);
	DistM_t tmpc(ctxt(), _B10.cols(), n);
	gemm(Trans::C, Trans::N, scalar_t(1.), _B10, wctmp, scalar_t(0.), tmpc);
	...
	gemm(Trans::N, Trans::N, scalar_t(-1.), Theta, tmpr, scalar_t(1.), Sloc);
	...
	gemm(Trans::N, Trans::N, scalar_t(1.), DUB01, wrtmp, scalar_t(0.), tmpr);
	DistM_t VB10t(ctxt(), Vhat.rows(), _B10.rows());
	gemm(Trans::N, Trans::C, scalar_t(1.), Vhat, _B10, scalar_t(0.), VB10t);
	DistM_t tmpc(ctxt(), Vhat.rows(), n);
	gemm(Trans::N, Trans::N, scalar_t(1.), VB10t, wctmp, scalar_t(0.), tmpc);
	ch1->apply_bwd(R_br, scalar_t(0.), Sr_br, wr, true);
	ch1->applyT_bwd(R_br, scalar_t(0.), Sc_br, wc, true);
	...
	Sc_br.from_block_row(RS_ch1);
	copy(ch1->rows(), Sc.cols(), RS_ch1, 0, 0, Sloc, 0, 0, _ctxt_all);
	gemm(Trans::N, Trans::N, scalar_t(-1.), Phi, tmpc, scalar_t(1.), Sloc);
	copy(ch1->rows(), Sc.cols(), Sloc, 0, 0, Sc, 0, 0, _ctxt_all);
      }
    }
	...
	if (this->U_rank() && Uop.cols()) {
	  ...
	  Uop0 = DistM_t(this->_ch[0]->ctxt(Theta.ctxt_loc()), this->_ch[0]->U_rank(), Uop.cols());
	  Uop1 = DistM_t(this->_ch[1]->ctxt(Theta.ctxt_loc()), this->_ch[1]->U_rank(), Uop.cols());
	  copy(this->_ch[0]->U_rank(), Uop.cols(), tmp, 0, 0, Uop0, 0, 0, _ctxt_all);
	  copy(this->_ch[1]->U_rank(), Uop.cols(), tmp, this->_ch[0]->U_rank(), 0, Uop1, 0, 0, _ctxt_all);
	  Uop.clear();
	}
	if (this->V_rank() && Vop.cols()) {
	  auto tmp = _V.apply(Vop);
	  Vop0 = DistM_t(this->_ch[0]->ctxt(Phi.ctxt_loc()), this->_ch[0]->V_rank(), Vop.cols());
	  Vop1 = DistM_t(this->_ch[1]->ctxt(Phi.ctxt_loc()), this->_ch[1]->V_rank(), Vop.cols());
	  copy(this->_ch[0]->V_rank(), Vop.cols(), tmp, 0, 0, Vop0, 0, 0, _ctxt_all);
	  copy(this->_ch[1]->V_rank(), Vop.cols(), tmp, this->_ch[0]->V_rank(), 0, Vop1, 0, 0, _ctxt_all);
	  Vop.clear();
	}
