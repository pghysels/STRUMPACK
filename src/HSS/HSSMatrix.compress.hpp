...
namespace strumpack {
  namespace HSS {
	...
	if (opts.verbose()) std::cout << "# compressing with d = " << d-opts.dd()
				      << " + " << opts.dd() << " (original)" << std::endl;
	compress_recursive_original(Rr, Rc, Sr, Sc, Aelem, opts, w, d-d_old, this->_openmp_task_depth);
	d_old = d;
	d = 2 * (d_old - opts.dd()) + opts.dd();
      }
    }

    template<typename scalar_t> void HSSMatrix<scalar_t>::compress_recursive_original
...
#pragma omp task default(shared) final(isfinal) mergeable
	  this->_ch[0]->compress_recursive_original(Rr, Rc, Sr, Sc, Aelem, opts, w.c[0], dd, depth+1);
#pragma omp task default(shared) final(isfinal) mergeable
	  this->_ch[1]->compress_recursive_original(Rr, Rc, Sr, Sc, Aelem, opts, w.c[1], dd, depth+1);
#pragma omp taskwait
...
#pragma omp taskwait
    ...
    template<typename scalar_t> void HSSMatrix<scalar_t>::compress_level_original
	...
	if (w.lvl < lvl) {
	  // TODO tasking
	  this->_ch[0]->compress_level_original(Rr, Rc, Sr, Sc, opts, w.c[0], dd, lvl, depth+1);
	  this->_ch[1]->compress_level_original(Rr, Rc, Sr, Sc, opts, w.c[1], dd, lvl, depth+1);
	  return;
	}
	if (!this->_ch[0]->is_compressed() || !this->_ch[1]->is_compressed()) return;
      }
    ...
    template<typename scalar_t> void HSSMatrix<scalar_t>::get_extraction_indices
	...
	if (w.lvl == lvl && this->is_untouched()) {
	  ...
	  for (std::size_t i=0; i<this->rows(); i++) I.back().push_back(i+w.offset.first+off.first);
	  for (std::size_t j=0; j<this->cols(); j++) J.back().push_back(j+w.offset.second+off.second);
	}
      } else {
	w.split(this->_ch[0]->dims());
	if (w.lvl < lvl) {
    ...
    (const elem_t& Aelem, const opts_t& opts, WorkCompress<scalar_t>& w, int lvl) {
      if (this->leaf()) {
	if (w.lvl < lvl) return;
	if (this->is_untouched()) {
	  std::vector<std::size_t> I, J;
    ...
    template<typename scalar_t> void HSSMatrix<scalar_t>::compute_local_samples
    (DenseM_t& Rr, DenseM_t& Rc, DenseM_t& Sr, DenseM_t& Sc, WorkCompress<scalar_t>& w, int d0, int d, int depth) {
      auto f0 = params::flops;
#pragma omp task default(shared) if(depth < params::task_recursion_cutoff_level) \
  final(depth >= params::task_recursion_cutoff_level-1) mergeable
	...
	if (this->leaf()) {
	  ...
	  gemm(Trans::N, Trans::N, scalar_t(-1), _D, wRr, scalar_t(1.), wSr, depth);
	} else {
	  DenseMW_t wSr0(this->_ch[0]->U_rank(), d, Sr, w.offset.second, d0);
	  DenseMW_t wSr1(this->_ch[1]->U_rank(), d, Sr, w.offset.second+this->_ch[0]->U_rank(), d0);
	  DenseMW_t wSr_ch0(this->_ch[0]->U_rows(), d, Sr, w.c[0].offset.second, d0);
	  DenseMW_t wSr_ch1(this->_ch[1]->U_rows(), d, Sr, w.c[1].offset.second, d0);
	  auto tmp0 = wSr_ch0.extract_rows(w.c[0].Jr);
	  auto tmp1 = wSr_ch1.extract_rows(w.c[1].Jr);
	  wSr0.copy(tmp0);
	  wSr1.copy(tmp1);
	  DenseMW_t wRr1(this->_ch[1]->V_rank(), d, Rr, w.c[1].offset.second, d0);
	  DenseMW_t wRr0(this->_ch[0]->V_rank(), d, Rr, w.c[0].offset.second, d0);
	  gemm(Trans::N, Trans::N, scalar_t(-1.), _B01, wRr1, scalar_t(1.), wSr0, depth);
	  gemm(Trans::N, Trans::N, scalar_t(-1.), _B10, wRr0, scalar_t(1.), wSr1, depth);
	}
      }
#pragma omp task default(shared) if(depth < params::task_recursion_cutoff_level) \
  final(depth >= params::task_recursion_cutoff_level-1) mergeable
	...
	if (this->leaf()) {
	  ...
	  gemm(Trans::C, Trans::N, scalar_t(-1), _D, wRc, scalar_t(1.), wSc, depth);
	} else {
	  DenseMW_t wSc0(this->_ch[0]->V_rank(), d, Sc, w.offset.second, d0);
	  DenseMW_t wSc1(this->_ch[1]->V_rank(), d, Sc, w.offset.second+this->_ch[0]->V_rank(), d0);
	  DenseMW_t wSc_ch0(this->_ch[0]->V_rows(), d, Sc, w.c[0].offset.second, d0);
	  DenseMW_t wSc_ch1(this->_ch[1]->V_rows(), d, Sc, w.c[1].offset.second, d0);
	  auto tmp1 = wSc_ch1.extract_rows(w.c[1].Jc);
	  auto tmp0 = wSc_ch0.extract_rows(w.c[0].Jc);
	  wSc0.copy(tmp0);
	  wSc1.copy(tmp1);
	  DenseMW_t wRc1(this->_ch[1]->U_rank(), d, Rc, w.c[1].offset.second, d0);
	  DenseMW_t wRc0(this->_ch[0]->U_rank(), d, Rc, w.c[0].offset.second, d0);
	  gemm(Trans::C, Trans::N, scalar_t(-1.), _B10, wRc1, scalar_t(1.), wSc0, depth);
	  gemm(Trans::C, Trans::N, scalar_t(-1.), _B01, wRc0, scalar_t(1.), wSc1, depth);
	}
      }
#pragma omp taskwait
    ...
    (DenseM_t& Sr, DenseM_t& Sc, const opts_t& opts, WorkCompress<scalar_t>& w, int d, int depth) {
      auto f0 = params::flops;
#pragma omp task default(shared) if(depth < params::task_recursion_cutoff_level) \
  final(depth >= params::task_recursion_cutoff_level-1) mergeable
      {
	...
	wSr.ID_row(_U.E(), _U.P(), w.Jr, opts.rel_tol(), opts.abs_tol(), opts.max_rank(), depth);
      }
#pragma omp task default(shared) if(depth < params::task_recursion_cutoff_level) \
  final(depth >= params::task_recursion_cutoff_level-1) mergeable
      {
	...
	wSc.ID_row(_V.E(), _V.P(), w.Jc, opts.rel_tol(), opts.abs_tol(), opts.max_rank(), depth);
      }
#pragma omp taskwait
	  ...
	  (int(_U.cols()) < d - opts.dd() && int(_V.cols()) < d - opts.dd())) {
    ...
    (DenseM_t& Rr, DenseM_t& Rc, WorkCompress<scalar_t>& w, int d0, int d, int depth) {
      auto f0 = params::flops;
#pragma omp task default(shared) if(depth < params::task_recursion_cutoff_level) \
  final(depth >= params::task_recursion_cutoff_level-1) mergeable
      {
