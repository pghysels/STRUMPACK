...
namespace strumpack {
  ...
  (integer_t n, idx_t* xadj, idx_t* adjncy, idx_t* vwgt, idx_t seps,
   idx_t* options, integer_t* perm, integer_t* iperm, idx_t* sizes) {
    ...
    auto iorder = order + n;
    int ierr = METIS_NodeNDP(n, xadj, adjncy, vwgt, seps, options, order, iorder, sizes);
    std::copy(order, order+n, perm);
    std::copy(iorder, iorder+n, iperm);
    delete[] order;
    return ierr;
  }
  template<> inline int WRAPPER_METIS_NodeNDP(idx_t n, idx_t* xadj, idx_t* adjncy, idx_t* vwgt, idx_t seps,
					      idx_t* options, idx_t* perm, idx_t* iperm, idx_t* sizes) {
    return METIS_NodeNDP(n, xadj, adjncy, vwgt, seps, options, perm, iperm, sizes);
  }
  ...
  template<> inline int WRAPPER_METIS_NodeND(idx_t n, idx_t* xadj, idx_t* adjncy, idx_t* vwgt,
  ...
  (idx_t* nvtxs, idx_t* ncon, std::vector<integer_t>& sep_csr_ptr, std::vector<integer_t>& sep_csr_ind,
   idx_t* two, idx_t* edge_cut, idx_t* partitioning) {
    ...
    int ierr = METIS_PartGraphRecursive(nvtxs, ncon, ptr, ind, NULL, NULL, NULL,
					two, NULL, NULL, NULL, edge_cut, partitioning);
    delete[] ptr;
    return ierr;
  }
  template<> inline int WRAPPER_METIS_PartGraphRecursive
  (idx_t* nvtxs, idx_t* ncon, std::vector<idx_t>& sep_csr_ptr, std::vector<idx_t>& sep_csr_ind,
   idx_t* two, idx_t* edge_cut, idx_t* partitioning) {
    return METIS_PartGraphRecursive(nvtxs, ncon, sep_csr_ptr.data(), sep_csr_ind.data(),
  ...
  sep_tree_from_metis_sizes(integer_t nodes, integer_t separators, idx_t* sizes) {
    std::unique_ptr<SeparatorTree<integer_t>> sep_tree(new SeparatorTree<integer_t>(nodes));
  ...
  metis_nested_dissection(CompressedSparseMatrix<scalar_t,integer_t>* A,
    ...
    xadj[n] = e;
    if (e==0) if (mpi_root()) std::cerr << "# WARNING: matrix seems to be diagonal!" << std::endl;
    int ierr;
    std::unique_ptr<SeparatorTree<integer_t>> sep_tree;
    if (opts.use_METIS_NodeNDP()) {
      integer_t nodes = std::max(integer_t(3), ( n / opts.nd_param() ) / 2 * 2 + 1);
      integer_t separators = nodes / 2;
      auto sizes = new idx_t[nodes + 1];
      ierr = WRAPPER_METIS_NodeNDP(n, xadj, adjncy, NULL, separators + 1, NULL, iperm, perm, sizes);
      delete[] xadj;
    ...
    if (ierr != METIS_OK) {
      std::cerr << "# ERROR: Metis nested dissection reordering failed with error code " << ierr << std::endl;
      // TODO throw an exception
    }
    return sep_tree;
  }
