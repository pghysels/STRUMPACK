...
/**
 * \file HBSOptions.hpp
 * \brief Contains the HBSOptions class as well as general routines
 * for HBS options.
 */
#ifndef HBS_OPTIONS_HPP
#define HBS_OPTIONS_HPP
...
namespace strumpack {
  ...
  namespace HBS {
    ...
    /**
     * Get the default relative HBS compression tolerance (this is for
     * double precision, might be overloaded depending on floating
     * point precision). This can be changed using the HBSOptions
     * object. Tuning this parameter (in the HBSOptions object) is
     * crucial for performance of the HBS algorithms.
     */
    template<typename real_t> inline real_t default_HBS_rel_tol() {
      return real_t(1e-2);
    }
    /**
     * Get the default absolute HBS compression tolerance (this is for
     * double precision, might be overloaded for single
     * precision). This can be changed using the HBSOptions object.
     */
    template<typename real_t> inline real_t default_HBS_abs_tol() {
      return real_t(1e-8);
    }

    /**
     * Get the default relative HBS compression tolerance for single
     * precision computations. This can be changed using the
     * HBSOptions object. Tuning this parameter (in the
     * HBSOptions<float> object) is crucial for performance of the HBS
     * algorithms.
     */
    template<> inline float default_HBS_rel_tol() {
      return 1e-1;
    }
    /**
     * Get the default absolute HBS compression tolerance for single
     * precision computations. This can be changed using the
     * HBSOptions object.
     */
    template<> inline float default_HBS_abs_tol() {
      return 1e-5;
    }

    /**
     * Enumeration of possible versions of the randomized sampling HBS
    ...
    /**
     * \class HBSOptions
     * \brief Class containing several options for the HBS code and
     * data-structures
     *
     ...
     */
    template<typename scalar_t> class HBSOptions
    ...
    public:
      ...
      HBSOptions() :
        structured::StructuredOptions<scalar_t>(structured::Type::HBS) {
        set_defaults();
      }

      HBSOptions(const structured::StructuredOptions<scalar_t>& sopts)
        : structured::StructuredOptions<scalar_t>(sopts) {
        this->type_ = structured::Type::HBS;
      }

      /**
       * Set the initial number of random samples to be used in the
       * random sampling HBS construction algorithm. See the manual
       ...
       * samples vectors in the adaptive randomized HBS compression
       ...
       * HBS compression algorithm for kernel matrices.
       *
       ...
       * neighbors search algorithm used in the HBS compression
      ...
      /**
       * Log the HBS ranks to a file. TODO is this currently
       * supported??
       */
       ...
       * adaptive randomized HBS compression. See the manual for more
       * info.
       *
       ...
       * \return Variant of HBS compression algorithm
       ...
       * constructing an HBS approximation of a kernel matrix.
       ...
       * used in the random sampling based HBS construcion,
       * him/her-self.
    ...
    private:
      ...
      void set_defaults() {
        this->type_ = structured::Type::HBS;
        this->rel_tol_ = default_HBS_rel_tol<real_t>();
        this->abs_tol_ = default_HBS_abs_tol<real_t>();
        this->leaf_size_ = 512;
