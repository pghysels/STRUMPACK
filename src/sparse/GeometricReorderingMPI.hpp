      ...
      if (dims[d] < 3 || N <= nd_param) {
        ...
        if (dsep) {
          if (dsep_leaf) {
            ...
            if (dist_nbsep)
              dist_tree.emplace_back
                (dist_tree.back().sep_end + N/dims[d], -1,
                 dist_left_root_id, dist_nbsep-1);
            else
              dist_tree.emplace_back
                (N/dims[d], -1, dist_left_root_id, dist_nbsep-1);
          }
          dist_nbsep++;
        }
        if (is_local) {
          if (local_nbsep)
            local_tree.emplace_back
              (local_tree.back().sep_end + N/dims[d], -1,
               local_left_root_id, local_nbsep-1);
          else local_tree.emplace_back
                 (N/dims[d], -1, local_left_root_id, local_nbsep-1);
          local_nbsep++;
        }
      }
