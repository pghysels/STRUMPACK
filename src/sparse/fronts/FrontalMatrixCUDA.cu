...
//             atomicMin(piv+k, i+1);
    ...
    gemmNN_block_kernel(int m, int n, int k,
    ...
    gemmNN_block_kernel_batched(int nrhs, FrontData<T>* dat) {
      FrontData<T>& A = dat[blockIdx.x];
      gemmNN_block_kernel<T,NT,alpha,beta>
        (A.n2, nrhs, A.n1, A.F21, A.F12, A.F22);
    }

    /**
     * Assume a (1xNT) threadblock, k <= NT
     */
    template<typename T, int NT, int alpha, int beta> __device__ void
    gemvN_block_kernel(int m, int k,
                       T* Aglobal, T* Bglobal, T* Cglobal) {
      using cuda_primitive_t = typename primitive_type<T>::value_type;
    ...
    template<typename T, int NT, int alpha, int beta> __global__ void
    gemvN_block_kernel_batched(FrontData<T>* dat) {
      FrontData<T>& A = dat[blockIdx.x];
      gemvN_block_kernel<T,NT,alpha,beta>
    ...
    template<typename T, int NT> void
    fwd_solve_block_batch(int nrhs, unsigned int count,
                          FrontData<T>* dat) {
      if (!count) return;
      using T_ = typename cuda_type<T>::value_type;
      auto dat_ = reinterpret_cast<FrontData<T_>*>(dat);
      ...
      if (nrhs == 1) {
        dim3 block1(1, NT);
        gemvN_block_kernel_batched<T_,NT,-1,1><<<count, block1>>>(dat_);
      } else
        gemmNN_block_kernel_batched<T_,NT,-1,1><<<count, block>>>(nrhs, dat_);
    }

    template<typename T, int NT> void
    bwd_solve_block_batch(int nrhs, unsigned int count,
                          FrontData<T>* dat) {
      if (!count) return;
      using T_ = typename cuda_type<T>::value_type;
      auto dat_ = reinterpret_cast<FrontData<T_>*>(dat);
    ...
    template void fwd_solve_block_batch<float,8>(int, unsigned int, FrontData<float>*);
    template void fwd_solve_block_batch<double,8>(int, unsigned int, FrontData<double>*);
    template void fwd_solve_block_batch<std::complex<float>,8>(int, unsigned int, FrontData<std::complex<float>>*);
    template void fwd_solve_block_batch<std::complex<double>,8>(int, unsigned int, FrontData<std::complex<double>>*);

    template void fwd_solve_block_batch<float,16>(int, unsigned int, FrontData<float>*);
    template void fwd_solve_block_batch<double,16>(int, unsigned int, FrontData<double>*);
    template void fwd_solve_block_batch<std::complex<float>,16>(int, unsigned int, FrontData<std::complex<float>>*);
    template void fwd_solve_block_batch<std::complex<double>,16>(int, unsigned int, FrontData<std::complex<double>>*);

    template void fwd_solve_block_batch<float,32>(int, unsigned int, FrontData<float>*);
    template void fwd_solve_block_batch<double,32>(int, unsigned int, FrontData<double>*);
    template void fwd_solve_block_batch<std::complex<float>,32>(int, unsigned int, FrontData<std::complex<float>>*);
    template void fwd_solve_block_batch<std::complex<double>,32>(int, unsigned int, FrontData<std::complex<double>>*);
