...
namespace strumpack {
  namespace gpu {
    ...
    real_type<thrust::complex<T>> { typedef T value_type; };
    template<class T> struct
    real_type<std::complex<T>> { typedef T value_type; };
    ...
    template<> struct
    primitive_type<thrust::complex<float>> { typedef float2 value_type; };
    template<> struct
    primitive_type<thrust::complex<double>> { typedef double2 value_type; };
    template<> struct
    primitive_type<std::complex<float>> { typedef float2 value_type; };
    template<> struct
    primitive_type<std::complex<double>> { typedef double2 value_type; };
    ...
    template<class T> struct
    cuda_type<std::complex<T>> { typedef thrust::complex<T> value_type; };


    /**
     * Zero out the frontal matrices, these can be all the
     * fronts on a level. The pointers and the sizes are stored in the
     * dat AssembleData<T> array.  This does not work for
     * std::complex<T>.
     *
     * This cab be done simpler/faster with cudaMemset.
     */
    // template<typename T> __global__ void
    // zero_kernel(unsigned int by0, AssembleData<T>* dat) {
    //   int x = blockIdx.x * blockDim.x + threadIdx.x,
    //     y = (blockIdx.y + by0) * blockDim.y + threadIdx.y;
    //   auto& F = dat[blockIdx.z];
    //   int d1 = F.d1, d2 = F.d2;
    //   if (x < d1 && y < d1) F.F11[y+x*d1] = T(0.);
    //   if (x < d2 && y < d1) F.F12[y+x*d1] = T(0.);
    //   if (x < d1 && y < d2) F.F21[y+x*d2] = T(0.);
    //   if (x < d2 && y < d2) F.F22[y+x*d2] = T(0.);
    // }
    // template<typename T> __global__ void
    // zero_kernel(unsigned int by0, AssembleData<std::complex<T>>* dat) {
    //   // TODO
    // }
