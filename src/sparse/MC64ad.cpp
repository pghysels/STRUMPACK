...
namespace strumpack {
  ...
  /**
     Modified (March 2, 2022) by P. Ghysels to make int_t a template
     parameter.
  */
...
#define min(a,b) ((a) < (b)) ? (a) : (b)
  ...
  int_t mc21ad_(int_t *, int_t *, int_t *,
                int_t *, int_t *, int_t *, int_t *, int_t *);
  template<typename int_t>
  int_t mc64bd_(int_t *, int_t *, int_t *, int_t *, double *, int_t *,
                int_t *, int_t *, int_t *, int_t *, int_t *, double *);
  template<typename int_t>
  int_t mc64rd_(int_t *, int_t *, int_t *, int_t *, double *);
  template<typename int_t>
  int_t mc64sd_(int_t *, int_t *, int_t *, int_t *, double *,
                int_t *, int_t *, int_t *, int_t *,
                int_t *, int_t *, int_t *, int_t *, int_t *);
  template<typename int_t>
  int_t mc64wd_(int_t *, int_t *, int_t *, int_t *, double *, int_t *,
                int_t *, int_t *, int_t *, int_t *, int_t *, int_t *,
                double *, double *);
  template<typename int_t>
  int_t mc64dd_(int_t *, int_t *, int_t *, double *, int_t *, int_t *);
  template<typename int_t>
  int_t mc64ed_(int_t *, int_t *, int_t *, double *, int_t *, int_t *);
  template<typename int_t>
  int_t mc64fd_(int_t *, int_t *, int_t *, int_t *,
                double *, int_t *, int_t *);
  template<typename int_t>
  int_t mc64qd_(int_t *, int_t *, int_t *,
                int_t *, int_t *, double *, int_t *, double *);
  template<typename int_t>
  int_t mc64ud_(int_t *, int_t *, int_t *, int_t *, int_t *,
                int_t *, int_t *, int_t *, int_t *, int_t *, int_t *,
                int_t *, int_t *, int_t *, int_t *);

  /* Subroutine */
  template<typename int_t> int_t mc64id(int_t *icntl)
  {
    int_t i__;
  ...
  /* Subroutine */
  template<typename int_t>
  int_t mc64ad(int_t *job, int_t *n, int_t *ne, int_t *ip,
               int_t *irn, double *a, int_t *num, int_t *cperm,
               ...
               int_t *icntl, int_t *info)
  {
    ...
    if (*job < 1 || *job > 5) {
      ...
      if (icntl[1] >= 0) {
        ...
        //        " because JOB = %d\n", info[1], *job);
        std::cout << " ****** Error in MC64A/AD. INFO(1) = " << info[1]
                  << " because JOB = " << *job << "\n";
      }
      goto L99;
    }
    ...
    if (*liw < k) {
      ...
      if (icntl[1] >= 0) {
        ...
        std::cout << " ****** Error in MC64A/AD. INFO(1) = " << info[1]
                  << " LIW too small, must be at least " << k << "\n";
      }
      goto L99;
    }
    /* Check LDW */
    ...
    if (*job > 1) {
      ...
      if (*ldw < k) {
        ...
        if (icntl[1] >= 0) {
          ...
          std::cout << " ****** Error in MC64A/AD. INFO(1) = " << info[1]
                    << " LDW too small, must be at least " << k << "\n";
        }
        goto L99;
      }
    }
        ...
        for (k = ip[j]; k <= i__2; ++k) {
          ...
          if (i__ < 1 || i__ > *n) {
            ...
            if (icntl[1] >= 0) {
              ...
              std::cout << " ****** Error in MC64A/AD. INFO(1) = " << info[1]
                        << " Column " << j
                        << " contains an entry with invalid row index " << i__ << "\n";
            }
            goto L99;
          }
          ...
          if (iw[i__] == j) {
            ...
            if (icntl[1] >= 0) {
              ...
              std::cout << " ****** Error in MC64A/AD. INFO(1) = " << info[1]
                        << "        Column " << j
                        << " contains two or more entries with row index " << i__ << "\n";
            }
            goto L99;
          } else {
            iw[i__] = j;
    ...
    if (icntl[3] >= 0) {
      ...
      //        " N = %d, NE = %8d\n", *job, *n, *ne);
      // printf(" IP(1:N+1)   = ");
      std::cout << "  ****** Input parameters for MC64A/AD: JOB = "
                << *job << "," << " N = " << *n << ", NE = " << *ne << "\n";
      std::cout << " IP(1:N+1)   = ";
      for (j=1; j<=(*n+1); ++j) {
        // printf("%8d", ip[j]);
    ...
    if (*job == 1) {
      ...
      std::cout << " ****** Warning from MC64A/AD. Need to link mc21ad.\n";
#endif
      goto L90;
    }
    ...
    if (*job == 2) {
      ...
      mc64bd_(n, ne, &ip[1], &irn[1], &a[1], &cperm[1], num, &iw[1],
              &iw[*n + 1], &iw[(*n << 1) + 1], &iw[*n * 3 + 1], &dw[1]);
      goto L90;
    }
    /* Compute bottleneck matching */
    if (*job == 3) {
    ...
    if (*job == 5) {
      ...
      for (j = 1; j <= i__1; ++j) {
        ...
        for (k = ip[j]; k <= i__2; ++k) {
          if (dw[*n * 3 + k] != 0.) {
            dw[*n * 3 + k] = fact - std::log(dw[*n * 3 + k]);
          } else {
            dw[*n * 3 + k] = rinf / *n;
          }
          /* L70: */
      ...
      mc64wd_(n, ne, &ip[1], &irn[1], &dw[*n * 3 + 1], &cperm[1], num,
              &iw[1], &iw[*n + 1], &iw[(*n << 1) + 1], &iw[*n * 3 + 1],
              &iw[(*n << 2) + 1], &dw[1], &dw[*n + 1]);
      if (*num == *n) {
        i__1 = *n;
  ...
  L90:
    if (info[1] == 0 && *num < *n) {
      ...
      if (icntl[2] >= 0) {
        ...
        std::cout << " ****** Warning from MC64A/AD. INFO(1) = " << info[1]
                  << " The matrix is structurally singular.\n";
      }
    }
    if (info[1] == 2) {
    ...
    if (icntl[3] >= 0) {
      ...
      // printf(" NUM        = %2d", *num);
      // printf(" CPERM(1:N) = ");
      std::cout << " ****** Output parameters for MC64A/AD: INFO(1:2)  = "
                << info[1] << " " << info[2] << "\n";
      std::cout << " NUM        = " << *num;
      std::cout << " CPERM(1:N) = ";
      for (j=1; j<=*n; ++j) {
  ...
  /* Subroutine */
  template<typename int_t>
  int_t mc64bd_(int_t *n, int_t *ne, int_t *ip, int_t *irn,
                double *a, int_t *iperm, int_t *num, int_t *jperm,
  ...
  /* Subroutine */
  template<typename int_t>
  int_t mc64dd_(int_t *i__, int_t *n, int_t *q, double *d__,
                int_t *l, int_t *iway)
  {
    /* System generated locals */
    int_t i__1;
  ...
  /* Subroutine */
  template<typename int_t>
  int_t mc64sd_(int_t *n, int_t *ne, int_t *ip, int_t *irn,
                double *a, int_t *iperm, int_t *numx, int_t *w,
  ...
  int_t mc64qd_(int_t *ip, int_t *lenl, int_t *lenh,
                int_t *w, int_t *wlen, double *a,
                int_t *nval, double *val)
  {
    /* System generated locals */
    int_t i__1, i__2, i__3;
  ...
  int_t mc64ud_(int_t *id, int_t *mod, int_t *n, int_t *
                irn, int_t *lirn, int_t *ip, int_t *lenc, int_t *fc,
                int_t *iperm, int_t *num, int_t *numx, int_t *pr,
                int_t *arp, int_t *cv, int_t *out)
  {
    /* System generated locals */
    int_t i__1, i__2, i__3, i__4;
  ...
  template int mc64ad(int *job, int *n, int *ne, int *ip,
                      int *irn, double *a, int *num, int *cperm,
                      int *liw, int *iw, int *ldw, double *dw,
                      int *icntl, int *info);
  template long int mc64ad(long int *job, long int *n, long int *ne, long int *ip,
                      long int *irn, double *a, long int *num, long int *cperm,
                      long int *liw, long int *iw, long int *ldw, double *dw,
                      long int *icntl, long int *info);
  template long long int mc64ad(long long int *job, long long int *n, long long int *ne, long long int *ip,
                                long long int *irn, double *a, long long int *num, long long int *cperm,
                                long long int *liw, long long int *iw, long long int *ldw, double *dw,
                                long long int *icntl, long long int *info);

  template int mc64id(int *icntl);
  template long int mc64id(long int *icntl);
  template long long int mc64id(long long int *icntl);
}
