...
namespace strumpack {
  ...
  public:
    ...
    CompressedSparseMatrix(integer_t n, integer_t* ptr, integer_t* ind, scalar_t* values, bool symm_sparsity);
    virtual ~CompressedSparseMatrix();
    virtual CompressedSparseMatrix<scalar_t,integer_t>* clone() = 0;
    ...
    inline void set_symmetric_sparsity(bool symm_sparse=true) { symmetric_sparsity = symm_sparse; }
    virtual void spmv(scalar_t* x, scalar_t* y) = 0;
    virtual void omp_spmv(scalar_t* x, scalar_t* y) = 0;
    virtual void permute(integer_t* iorder, integer_t* order);
    virtual void permute(std::vector<integer_t>& iorder, std::vector<integer_t>& order) {
      permute(iorder.data(), order.data());
    }
    virtual int permute_and_scale(int job, std::vector<integer_t>& perm, std::vector<scalar_t>& Dr,
				  std::vector<scalar_t>& Dc, bool apply=true);
    virtual void apply_scaling(std::vector<scalar_t>& Dr, std::vector<scalar_t>& Dc) = 0;
    virtual void apply_column_permutation(std::vector<integer_t>& perm) = 0;
    virtual void symmetrize_sparsity();
    virtual void extract_separator(integer_t sep_end, const std::vector<std::size_t>& I,
    				   const std::vector<std::size_t>& J, DenseM_t& B, int depth) const = 0;
    virtual void front_multiply(integer_t slo, integer_t shi, integer_t* upd, integer_t dupd,
				const DenseM_t& R, DenseM_t& Sr, DenseM_t& Sc) const = 0;
    virtual void extract_front(scalar_t* F11, scalar_t* F12, scalar_t* F21, integer_t dim_sep, integer_t dim_upd,
    			       integer_t sep_begin, integer_t sep_end, integer_t* upd, int depth) = 0;
    virtual void extract_F11_block(scalar_t* F, integer_t ldF, integer_t row, integer_t nr_rows,
    				   integer_t col, integer_t nr_cols) = 0;
    virtual void extract_F12_block(scalar_t* F, integer_t ldF, integer_t row, integer_t nr_rows,
    				   integer_t col, integer_t nr_cols, integer_t* upd) = 0;
    virtual void extract_F21_block(scalar_t* F, integer_t ldF, integer_t row, integer_t nr_rows,
    				   integer_t col, integer_t nr_cols, integer_t* upd) = 0;
    virtual void print();
    virtual void print_dense(std::string name) { std::cerr << "print_dense not implemented for this matrix type" << std::endl; }
    virtual void print_MM(std::string filename) { std::cerr << "print_MM not implemented for this matrix type" << std::endl; }
    virtual int read_matrix_market(std::string filename) = 0;
    bool has_symmetric_sparsity() { return symmetric_sparsity; }
    virtual real_t max_scaled_residual(scalar_t* x, scalar_t* b) = 0;
    virtual void strumpack_mc64(int_t job, int_t* num, integer_t* perm, int_t liw, int_t* iw, int_t ldw,
				double* dw, int_t* icntl, int_t* info) {}

    virtual void extract_separator_2d(integer_t sep_end, const std::vector<std::size_t>& I, const std::vector<std::size_t>& J,
				      DistM_t& B, MPI_Comm comm) const = 0;
    virtual void front_multiply_2d(integer_t sep_begin, integer_t sep_end, integer_t* upd, integer_t dim_upd,
				   DistM_t& R, DistM_t& Srow, DistM_t& Scol, int ctx_all, MPI_Comm R_comm, int depth) = 0;
  ...
  protected:
    ...
    integer_t* _ind;
    scalar_t* _val;
    bool symmetric_sparsity;

    enum MMsym {GENERAL, SYMMETRIC, SKEWSYMMETRIC, HERMITIAN};
    std::vector<std::tuple<integer_t,integer_t,scalar_t>> read_matrix_market_entries(std::string filename);
    void clone_data(const CompressedSparseMatrix<scalar_t,integer_t>& A);
    ...
    virtual bool is_mpi_root() { return mpi_root(); }
  };

  template<typename scalar_t,typename integer_t>
  CompressedSparseMatrix<scalar_t,integer_t>::CompressedSparseMatrix()
    : _n(0), _nnz(0), _ptr(NULL), _ind(NULL), _val(NULL), symmetric_sparsity(false) {
  }
  ...
  (const CompressedSparseMatrix<scalar_t,integer_t>& A) {
    _n = A.size();
    _nnz = A.nnz();
    symmetric_sparsity = A.symmetric_sparsity;
  ...
  template<typename scalar_t,typename integer_t>
  CompressedSparseMatrix<scalar_t,integer_t>::CompressedSparseMatrix(integer_t n, integer_t nnz)
    : _n(n), symmetric_sparsity(false) {
    _nnz = nnz;
    _ptr = new integer_t[_n+1];
    _ind = new integer_t[_nnz];
    _val = new scalar_t[_nnz];
  ...
  (integer_t n, integer_t* ptr, integer_t* ind, scalar_t* values, bool symm_sparsity)
    :_n(n), symmetric_sparsity(symm_sparsity) {
    _nnz = ptr[_n]-ptr[0];
    _ptr = new integer_t[_n+1];
    _ind = new integer_t[_nnz];
    _val = new scalar_t[_nnz];
    ...
    std::copy(values, values+_nnz, _val);
  }
  ...
  template<typename scalar_t,typename integer_t> void
  CompressedSparseMatrix<scalar_t,integer_t>::print() {
    if (!is_mpi_root()) return;
    std::cout << "size: " << size() << std::endl;
    std::cout << "nnz: " << nnz() << std::endl;
    std::cout << "ptr: " << std::endl << "\t";
    for (integer_t i=0; i<=size(); i++) std::cout << _ptr[i] << " ";
    std::cout << std::endl << "ind: ";
    for (integer_t i=0; i<nnz(); i++) std::cout << _ind[i] << " ";
    std::cout << std::endl << "val: ";
    for (integer_t i=0; i<nnz(); i++) std::cout << _val[i] << " ";
    std::cout << std::endl;
  }

  extern "C" {
    int_t strumpack_mc64id_(int_t*);
    int_t strumpack_mc64ad_(int_t*, int_t*, int_t*, int_t*, int_t*, double*, int_t*, int_t*, int_t*,
  ...
  (int job, std::vector<integer_t>& perm, std::vector<scalar_t>& Dr, std::vector<scalar_t>& Dc, bool apply) {
    ...
    if (job > 5 || job < 0) {
      if (is_mpi_root())
	std::cerr << "# WARNING: mc64 job " << job
		  << " not supported, I'm not doing any column permutation or matrix scaling!" << std::endl;
      return 1;
    }
    perm.resize(_n);
    int_t liw = 0;
    ...
    switch (info[0]) {
    case  0: break;
    case  1: if (is_mpi_root()) std::cerr << "# ERROR: matrix is structurally singular" << std::endl; delete[] dw; delete[] iw; return 1; break;
    case  2: if (is_mpi_root()) std::cerr << "# WARNING: mc64 scaling produced large scaling factors which may cause overflow!" << std::endl; break;
    default: if (is_mpi_root()) std::cerr << "# ERROR: mc64 failed with info[0]=" << info[0] << std::endl; delete[] dw; delete[] iw; return 1; break;
    }
    if (job == 5) { // scaling
      Dr.resize(_n);
      Dc.resize(_n);
#pragma omp parallel for
    ...
    if (apply) symmetric_sparsity = false;
    return 0;
  }

  template<typename scalar_t,typename integer_t> void
  CompressedSparseMatrix<scalar_t,integer_t>::symmetrize_sparsity() {
    if (symmetric_sparsity) return;
...
#pragma omp critical
  ...
  CompressedSparseMatrix<scalar_t,integer_t>::read_matrix_market_entries(std::string filename) {
    ...
    if (fgets(cline, max_cline, fp) == NULL) { if (is_mpi_root()) std::cerr << "ERROR: could not read from file" << std::endl; exit(1); }
    ...
    else if (strstr(cline, "symmetric")) { s = SYMMETRIC; symmetric_sparsity = true; }
    else if (strstr(cline, "hermitian")) { s = HERMITIAN; symmetric_sparsity = true; }
	...
	_n = static_cast<integer_t>(in);
	  ...
	  std::cout << "# reading " << number_format_with_commas(m) << " by "
		    ...
		    << number_format_with_commas(_nnz) << " nnz's from " << filename << std::endl;
  	if (m != _n) { if (is_mpi_root()) std::cerr << "ERROR: matrix is not square!" << std::endl; exit(1); }
  	break;
      }
    }
	...
	integer_t c = static_cast<integer_t>(ic);
    	...
    	if (r != c) {
    	  switch (s) {
    	  case SKEWSYMMETRIC: A.push_back(std::make_tuple(c, r, -v)); break;
    	  case SYMMETRIC:     A.push_back(std::make_tuple(c, r, v)); break;
    	  case HERMITIAN:     A.push_back(std::make_tuple(c, r, blas::my_conj(v))); break;
    	  default: break;
    	  }
    	}
      }
   ...
   */
  template<typename scalar_t,typename integer_t> void
  CompressedSparseMatrix<scalar_t,integer_t>::permute(integer_t* iorder, integer_t* order) {
...
#pragma omp parallel for
    for (integer_t i=0; i<_n; i++) {
      auto lb = new_ptr[i];
      auto ub = new_ptr[i+1];
      sort_indices_values<scalar_t,integer_t>(new_ind+lb, new_val+lb, 0, ub-lb);
    }
    set_ptr(new_ptr);
    set_ind(new_ind);
    set_val(new_val);
