...
namespace strumpack {
  ...
  public:
    ...
    Inertia inertia_node() override { return inertia_; }
  ...
  (const SpMat_t& A, const SPOptions<scalar_t>& opts,
   int etree_level, int task_depth) {
    if (this->dim_sep()) {
      ...
      DenseM_t F11c = DenseM_t(F11.rows(), F11.cols());
      F11c.zero(); // not sure if this is needed; definitely need a deep copy
      F11c.copy(F11);
      inertia_ = F11c.inertia();

      // Replaces LU for Schur complement with LDL using sytrf_rook()
      // For the LDL using sytrf() follow the commented lines
      // piv = F11.sytrf_rook();                 // piv = F11.sytrf();
      // if (this->dim_upd()) {
      //     sytrs_rook(UpLo::L, F11, piv, F12); // sytrs(UpLo::L, F11, piv, F12);
