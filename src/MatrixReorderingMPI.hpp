...
namespace strumpack {
  ...
  public:
    ...
    int nested_dissection(const SPOptions<scalar_t>& opts, CSRMatrixMPI<scalar_t,integer_t>* A,
    			  int nx, int ny, int nz);

    void separator_reordering(const SPOptions<scalar_t>& opts, CSRMatrixMPI<scalar_t,integer_t>* A,
  ...
  private:
    ...
    void allgather_local_order(CSRMatrixMPI<scalar_t,integer_t>* A_mpi, integer_t* local_order);
    void distributed_separator_reordering(const SPOptions<scalar_t>& opts, integer_t* global_sep_order);
    void local_separator_reordering(const SPOptions<scalar_t>& opts, integer_t* global_sep_order);
    void nested_dissection_print(const SPOptions<scalar_t>& opts, integer_t n, integer_t nnz);
  };

  template<typename scalar_t,typename integer_t>
  MatrixReorderingMPI<scalar_t,integer_t>::MatrixReorderingMPI(integer_t n, MPI_Comm c)
    : MatrixReordering<scalar_t,integer_t>(n), dsep_internal(0), dsep_leaf(0), _comm(c) {
  }
  ...
  (const SPOptions<scalar_t>& opts, CSRMatrixMPI<scalar_t,integer_t>* A, int nx, int ny, int nz) {
    if (!is_parallel(opts.reordering_method())) {
      auto rank = mpi_rank(_comm);
      auto P = mpi_nprocs(_comm);
      auto Aseq = A->gather(); // TODO only gather the graph? without the values or the diagonal
      std::unique_ptr<SeparatorTree<integer_t>> global_sep_tree;
      if (Aseq) { // only root
	switch (opts.reordering_method()) {
	case ReorderingStrategy::METIS: {
	  global_sep_tree = metis_nested_dissection(Aseq.get(), this->perm, this->iperm, opts); break; }
	case ReorderingStrategy::SCOTCH: {
	  global_sep_tree = scotch_nested_dissection(Aseq.get(), this->perm, this->iperm, opts); break; }
	case ReorderingStrategy::RCM: {
	  global_sep_tree = rcm_reordering(Aseq.get(), this->perm, this->iperm); break; }
	default: assert(true);
	}
	Aseq.reset();
	global_sep_tree->check();
      ...
      if (rank) global_sep_tree = std::unique_ptr<SeparatorTree<integer_t>>(new SeparatorTree<integer_t>(nbsep));
	...
	if (nx*ny*nz != A->size()) {
	  std::cerr << "# ERROR: Geometric reordering failed. \n"
	    ...
	    "# on a regular grid and you need to provide the mesh sizes." << std::endl;
	  return 1;
	}
	std::tie(this->sep_tree, local_sep_tree) = geometric_nested_dissection_dist
	  (nx, ny, nz, A->begin_row(), A->end_row(), _comm, this->perm, this->iperm, opts.nd_param()); break; }
      case ReorderingStrategy::PARMETIS: {
	this->sep_tree = parmetis_nested_dissection(A, _comm, true, this->perm, opts); break; }
      case ReorderingStrategy::PTSCOTCH: {
	this->sep_tree = ptscotch_nested_dissection(A, _comm, true, this->perm, opts); break; }
      default: assert(true);
      }
      this->sep_tree->check();
      get_local_graphs(A);
  ...
  (const SPOptions<scalar_t>& opts, CSRMatrixMPI<scalar_t,integer_t>* A, bool verbose) {
    auto n = A->size();
    ...
    for (integer_t i=0; i<n; i++) this->perm[global_sep_order[i]] = this->iperm[i];
  ...
  template<typename scalar_t,typename integer_t> void
  MatrixReorderingMPI<scalar_t,integer_t>::separator_reordering
  (const SPOptions<scalar_t>& opts, CSRMatrixMPI<scalar_t,integer_t>* A, FrontalMatrix<scalar_t,integer_t>* F) {
    auto N = A->size();
...
#pragma omp single
    ...
    std::cout << "TODO MatrixReorderingMPI::separator_reordering" << std::endl;
...
//     for (integer_t i=0; i<N; i++) iwork[i] = sorder[this->perm[i]];  // product of perm and sep_order
...
//     F->permute_upd_indices(sorder);
  ...
  (const SPOptions<scalar_t>& opts, integer_t* global_sep_order) {
    ...
    for (integer_t i=0; i<n_local; i++) local_sep_order[i] = i + sub_graph_range.first;
	...
	std::function<void(HSS::HSSPartitionTree&, integer_t&, integer_t, integer_t)> split =
	  [&](HSS::HSSPartitionTree& hss_tree, integer_t& nr_parts, integer_t part, integer_t count) {
	  my_sub_graph->extract_separator_subgraph
	  (opts.separator_ordering_level(), sub_graph_range.first, sep_begin, sep_end,
	   part, local_sep_order+sep_begin,  sep_csr_ptr, sep_csr_ind);
	  idx_t ncon = 1, edge_cut = 0, two = 2, nvtxs = sep_csr_ptr.size()-1;
	  auto partitioning = new idx_t[nvtxs];
	  int info = WRAPPER_METIS_PartGraphRecursive(&nvtxs, &ncon, sep_csr_ptr, sep_csr_ind,
	  ...
	  for (integer_t p=0; p<2; p++)
	    ...
	    else std::replace(local_sep_order+sep_begin, local_sep_order+sep_end, -count-p, nr_parts++);
	  delete[] partitioning;
	};
	std::fill(local_sep_order+sep_begin, local_sep_order+sep_end, integer_t(0));
	integer_t parts = 0;
	split(tree, parts, 0, 1);
	for (integer_t part=0, count=sep_begin+sub_graph_range.first; part<parts; part++)
	  for (integer_t i=sep_begin; i<sep_end; i++)
	    if (local_sep_order[i] == part) local_sep_order[i] = -count++;
	for (integer_t i=sep_begin; i<sep_end; i++) local_sep_order[i] = -local_sep_order[i];
      }
      local_sep_tree->HSS_trees()[sep] = tree;
      if (local_sep_tree->lch()[sep] != -1) reorder_separator(local_sep_tree->lch()[sep]);
      if (local_sep_tree->rch()[sep] != -1) reorder_separator(local_sep_tree->rch()[sep]);
    };
    if (local_sep_tree->separators() > 0) reorder_separator(local_sep_tree->root());
    my_sub_graph->clear_temp_data();
    for (integer_t i=0; i<n_local; i++) local_sep_iorder[local_sep_order[i]-sub_graph_range.first] = i;
    my_sub_graph->permute_local(local_sep_order, local_sep_iorder,
    ...
    MPI_Allgatherv(local_sep_order, rcnts[rank], mpi_type<integer_t>(),
     		   global_sep_order, rcnts, displs, mpi_type<integer_t>(), _comm);
    delete[] rcnts;
  ...
  (const SPOptions<scalar_t>& opts, integer_t* global_sep_order) {
	...
	std::function<void(HSS::HSSPartitionTree&, integer_t&, integer_t, integer_t)> split =
	  [&](HSS::HSSPartitionTree& hss_tree, integer_t& nr_parts, integer_t part, integer_t count) {
	  my_dist_sep->extract_separator_subgraph(opts.separator_ordering_level(), dsep_begin, 0, dim_dsep,
	  ...
	  for (integer_t p=0; p<2; p++)
	    if (hss_tree.c[p].size > 2 * opts.HSS_options().leaf_size())
	      split(hss_tree.c[p], nr_parts, -count-p, count+2);
	    else std::replace(dsep_order, dsep_order+dim_dsep, -count-p, nr_parts++);
	  delete[] partitioning;
	};
	...
	for (integer_t part=0, count=dsep_begin; part<parts; part++)
	  for (integer_t i=0; i<dim_dsep; i++)
	    if (dsep_order[i] == part) dsep_order[i] = -count++;
	for (integer_t i=0; i<dim_dsep; i++) dsep_order[i] = -dsep_order[i];
      } else for (integer_t i=0; i<dim_dsep; i++) dsep_order[i] = i+dsep_begin;
      this->sep_tree->HSS_trees()[dsep_internal] = tree;
      for (integer_t i=0; i<dim_dsep; i++) dsep_iorder[dsep_order[i]-dsep_begin] = i;
    }
    auto rcnts = new int[2*P];
    auto displs = rcnts + P;
    for (int p=0; p<P; p++) {
    ...
    MPI_Allgatherv(dsep_order, rcnts[rank], mpi_type<integer_t>(),
		   global_sep_order, rcnts, displs, mpi_type<integer_t>(), _comm);
    delete[] rcnts;
    my_dist_sep->permute_rows_local_cols_global(global_sep_order, dsep_iorder, dsep_begin, dsep_end);
    delete[] dsep_order;
  ...
  (CSRMatrixMPI<scalar_t,integer_t>* A) {
    ...
    for (integer_t sep=0, p_local=0, p_dist=0; sep<this->sep_tree->separators(); sep++) {
      if (this->sep_tree->lch()[sep] == -1) { // sep is a leaf, so it is the local graph of proces p
	if (p_local==rank) dsep_leaf = sep;
	sub_graph_ranges[p_local] = std::make_pair(this->sep_tree->sizes()[sep], this->sep_tree->sizes()[sep+1]);
	proc_dist_sep[sep] = p_local++;
      } else { // sep was computed using distributed nested dissection, assign it to process p_dist
	if (p_dist==rank) dsep_internal = sep;
	dist_sep_ranges[p_dist] = std::make_pair(this->sep_tree->sizes()[sep], this->sep_tree->sizes()[sep+1]);
  ...
  (CSRMatrixMPI<scalar_t,integer_t>* A) {
    ...
    auto sub_graph_etree = spsymetree(my_sub_graph->get_ptr(), my_sub_graph->get_ptr()+1, my_sub_graph->get_ind(),
				      sub_n, sub_graph_range.first);
    auto post_order = etree_postorder(sub_graph_etree);
    auto iwork = new integer_t[sub_n];
    for (integer_t i=0; i<sub_n; ++i) iwork[post_order[i]] = post_order[sub_graph_etree[i]];
    for (integer_t i=0; i<sub_n; ++i) sub_graph_etree[i] = iwork[i];
    local_sep_tree = std::unique_ptr<SeparatorTree<integer_t>>(new SeparatorTree<integer_t>(sub_graph_etree));
    ...
    MPI_Allgatherv(post_order.data(), rcnts[rank], mpi_type<integer_t>(), global_post_order,
     		   rcnts, displs, mpi_type<integer_t>(), _comm);
    delete[] rcnts;
    for (integer_t i=0; i<n; i++) this->iperm[this->perm[i]] = i;
    for (integer_t i=0; i<n; i++) this->perm[global_post_order[i]] = this->iperm[i];
  ...
  (const SPOptions<scalar_t>& opts, integer_t n, integer_t nnz) {
    if (opts.verbose()) {
      ...
      MPI_Allreduce(MPI_IN_PLACE, &total_separators, 1, mpi_type<integer_t>(), MPI_SUM, _comm);
      total_separators += std::max(integer_t(0), this->sep_tree->separators() - mpi_nprocs(_comm));
      auto max_level = local_sep_tree->levels() + this->sep_tree->level(dsep_leaf) - 1;
      MPI_Allreduce(MPI_IN_PLACE, &max_level, 1, mpi_type<integer_t>(), MPI_MAX, _comm);
      if (!mpi_rank(_comm)) {
	std::cout << "# initial matrix:" << std::endl;
	std::cout << "#   - number of unknowns = " << number_format_with_commas(n) << std::endl;
	std::cout << "#   - number of nonzeros = " << number_format_with_commas(nnz) << std::endl;
	std::cout << "# nested dissection reordering:" << std::endl;
	std::cout << "#   - " << get_name(opts.reordering_method()) << " reordering" << std::endl;
	if (opts.reordering_method() == ReorderingStrategy::METIS) {
	  if (opts.use_METIS_NodeNDP()) {
	    std::cout << "#      - used METIS_NodeNDP (iso METIS_NodeND)" << std::endl;
	    if (opts.use_MUMPS_SYMQAMD())
	      std::cout << "#      - supernodal tree was built using MUMPS_SYMQAMD "
			<< (opts.use_agg_amalg() ? "with" : "without") << " aggressive amalgamation" << std::endl;
	    else std::cout << "#      - supernodal tree from METIS_NodeNDP is used" << std::endl;
	  } else {
	    std::cout << "#      - used METIS_NodeND (iso METIS_NodeNDP)" << std::endl;
	    if (opts.use_MUMPS_SYMQAMD())
	      std::cout << "#      - supernodal tree was built using MUMPS_SYMQAMD "
			<< (opts.use_agg_amalg() ? "with" : "without") << " aggressive amalgamation" << std::endl;
	    else std::cout << "#      - supernodal tree was built from etree" << std::endl;
	  }
	}
	std::cout << "#   - strategy parameter = " << opts.nd_param() << std::endl;
	std::cout << "#   - number of separators = " << number_format_with_commas(total_separators) << std::endl;
