...
namespace strumpack {
  ...
  public:
    ...
    int nested_dissection(SPOptions<scalar_t>& opts, CSRMatrix<scalar_t,integer_t>* A,
     			  int nx, int ny, int nz);
    int nested_dissection(SPOptions<scalar_t>& opts, CSRMatrix<scalar_t,integer_t>* A,
     			  MPI_Comm comm, int nx, int ny, int nz);

    void separator_reordering(const SPOptions<scalar_t>& opts, CSRMatrix<scalar_t,integer_t>* A,
  ...
  protected:
    virtual void separator_reordering_print(integer_t max_nr_neighbours, integer_t max_dim_sep);
    virtual void nested_dissection_print(SPOptions<scalar_t>& opts, integer_t n, integer_t nnz, bool verbose);

  private:
    void split_separator(const SPOptions<scalar_t>& opts, HSS::HSSPartitionTree& hss_tree, integer_t& nr_parts, integer_t sep,
			 CSRMatrix<scalar_t,integer_t>* A, integer_t part,
			 integer_t count, integer_t* sorder);
    void extract_separator(const SPOptions<scalar_t>& opts, integer_t part, integer_t sep_beg, integer_t sep_end,
			   CSRMatrix<scalar_t,integer_t>* A,
			   std::vector<idx_t>& xadj, std::vector<idx_t>& adjncy,
			   integer_t* sorder);
    void separator_reordering_recursive(const SPOptions<scalar_t>& opts, CSRMatrix<scalar_t,integer_t>* A,
  ...
  (SPOptions<scalar_t>& opts, CSRMatrix<scalar_t,integer_t>* A, int nx, int ny, int nz) {
    switch (opts.reordering_method()) {
    case ReorderingStrategy::METIS: { sep_tree = metis_nested_dissection(A, perm, iperm, opts); break; }
    case ReorderingStrategy::SCOTCH: { sep_tree = scotch_nested_dissection(A, perm, iperm, opts); break; }
	...
	return 1;
      ...
      sep_tree = geometric_nested_dissection(nx, ny, nz, perm, iperm, opts.nd_param()); break; }
    case ReorderingStrategy::RCM: { sep_tree = rcm_reordering(A, perm, iperm); break; }
    default: std::cerr << "# ERROR: parallel matrix reorderings are not supported from this interface, \n"
	"\tuse StrumpackSparseSolverMPI or StrumpackSparseSolverMPIDist instead." << std::endl;
      return 1;
    }
  ...
  template<typename scalar_t,typename integer_t> int
  MatrixReordering<scalar_t,integer_t>::nested_dissection
  (SPOptions<scalar_t>& opts, CSRMatrix<scalar_t,integer_t>* A, MPI_Comm comm, int nx, int ny, int nz) {
	...
	default: assert(false);
      ...
      if (rank) sep_tree = std::unique_ptr<SeparatorTree<integer_t>>(new SeparatorTree<integer_t>(nbsep));
	...
	sep_tree = geometric_nested_dissection(nx, ny, nz, perm, iperm, opts.nd_param());
      } else {
	CSRMatrixMPI<scalar_t,integer_t> Ampi(A, comm, false);
	switch (opts.reordering_method()) {
	case ReorderingStrategy::PARMETIS: { parmetis_nested_dissection(&Ampi, comm, false, perm, opts); break; }
	case ReorderingStrategy::PTSCOTCH: { ptscotch_nested_dissection(&Ampi, comm, false, perm, opts); break; }
	default: assert(true);
	}
	sep_tree = build_sep_tree_from_perm(n, A->get_ptr(), A->get_ind(), perm, iperm);
      }
    }
    nested_dissection_print(opts, n, A->nnz(), opts.verbose() && !mpi_rank(comm));
    return 0;
  }
  ...
  (const SPOptions<scalar_t>& opts, CSRMatrix<scalar_t,integer_t>* A, bool verbose) {
    auto N = A->size();
...
#pragma omp single
    ...
    for (integer_t i=0; i<N; i++) iwork[i] = sorder[perm[i]];  // product of perm and sep_order
...
#pragma omp task firstprivate(sep) default(shared)
	separator_reordering_recursive(opts, A, is_hss, sep_tree->lch()[sep], sorder);
      }
...
#pragma omp critical
	...
	sep_tree->HSS_trees()[sep] = hss_tree; // not thread safe!
      ...
      if (sep_tree->lch()[sep] != -1) separator_reordering_recursive(opts, A, is_hss, sep_tree->lch()[sep], sorder);
      if (sep_tree->rch()[sep] != -1) separator_reordering_recursive(opts, A, is_hss, sep_tree->rch()[sep], sorder);
  ...
  (const SPOptions<scalar_t>& opts, HSS::HSSPartitionTree& hss_tree, integer_t& nr_parts, integer_t sep,
   CSRMatrix<scalar_t,integer_t>* A, integer_t part, integer_t count, integer_t* sorder) {
    auto sep_begin = sep_tree->sizes()[sep];
    auto sep_end = sep_tree->sizes()[sep+1];
	...
	split_separator(opts, hss_tree.c[p], nr_parts, sep, A, -count-p, count+2, sorder);
      else std::replace(sorder+sep_begin, sorder+sep_end, -count-p, nr_parts++);
  }

  template<typename scalar_t,typename integer_t> void
  MatrixReordering<scalar_t,integer_t>::extract_separator
  (const SPOptions<scalar_t>& opts, integer_t part, integer_t sep_begin, integer_t sep_end, CSRMatrix<scalar_t,integer_t>* A,
	  ...
	  if (lc >= 0 && lc < dim_sep && sorder[c]==part && !mark[lc]) {
	    ...
	    if (opts.separator_ordering_level() > 0) {
	      for (integer_t k=A->get_ptr()[c]; k<A->get_ptr()[c+1]; k++) {
	   	...
	   	if (cc!=i && lcc >= 0 && lcc < dim_sep && sorder[cc]==part && !mark[lcc]) {
	   	  mark[lcc] = true;
...
#pragma omp single
  ...
  (SPOptions<scalar_t>& opts, integer_t n, integer_t nnz, bool verbose) {
	...
	if (opts.use_METIS_NodeNDP()) {
      ...
      std::cout << "#   - number of separators = " << number_format_with_commas(sep_tree->separators()) << std::endl;
      std::cout << "#   - number of levels = " << number_format_with_commas(sep_tree->levels()) << std::endl;
    }
    if (opts.log_assembly_tree()) {
      std::string filename = "assembly_tree_" + get_name(opts.reordering_method());
      if (opts.reordering_method() == ReorderingStrategy::METIS) {
	if (opts.use_METIS_NodeNDP()) {
	  filename += "_NodeNDP";
	  if (opts.use_MUMPS_SYMQAMD())
  ...
  (integer_t max_nr_neighbours, integer_t max_dim_sep) {
    ...
    std::cout << "#   - maximum connectivity = " << max_nr_neighbours << std::endl;
    std::cout << "#   - maximum separator dimension = " << max_dim_sep << std::endl;
  }
